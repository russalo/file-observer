...
# discovery containment. On Windows rglob DESCENDS into junctions (unlike POSIX,
# where it omits symlinked dirs), so a file reached through one must be skipped to
# match POSIX semantics and avoid double-counting an in-tree junction's target.
_WINDOWS = os.name == "nt"


def _reached_through_reparse(path: Path, root: Path) -> bool:
    """True if any ancestor directory of ``path`` up to (excluding) ``root`` is a
    reparse point (NTFS junction / directory symlink). Windows-only helper.

    Bounded by tree depth; any error → treated as not-a-reparse. The
    resolve()-containment in ``iter_files`` is the security backstop, so failing
    open on this *skip* never enables a tree-escape — it only affects whether an
    in-tree junction's files are (correctly) omitted for POSIX parity.
    """
    ...
    for parent in path.parents:
        ...
        try:
            ...
            attrs = os.lstat(parent).st_file_attributes  # type: ignore[attr-defined]
    ...
    def iter_files(self, root: Path) -> Iterable[Path]:
        ...
        for path in sorted(root.rglob("*"), key=lambda p: p.as_posix()):
            if path.is_file():
                ...
                # mirroring _is_safe_zip_entry for ZIP paths (red-team #6, v1.8.1).
                if path.is_symlink():
                    try:
                ...
                # v1.46: on Windows, rglob DESCENDS into junction/reparse-point
                # directories (they are NOT symlinks, so is_symlink() above misses
                # files reached through them) — unlike POSIX, where rglob does not
                # follow symlinked dirs. Skip a file reached THROUGH a reparse-point
                # dir so the observed file SET matches POSIX: this closes the
                # tree-escape via an out-of-tree junction (its files are omitted,
                # never read) AND kills the in-tree-junction double-count. Windows-
                # only, and left AS the v1.8.1 symlink path for POSIX — so POSIX
                # behavior is completely unchanged (a mid-scan-deleted normal file
                # still becomes an ErrorRecord, never a silent omission). The
                # ancestor walk only costs stats, incurred on Windows alone.
