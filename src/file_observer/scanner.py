...
extracts metadata and signals, emits a deterministic JSON manifest.
    ...
    Schema:     1.25
    Python:     >= 3.12
    Spec:       docs/v1.50.0_RFC_Specification.md (current)
...
# v1.36: XML hardening via purexml (pure-stdlib, zero-dep, oracle-gated defusedxml replacement; MIT).
# fo OPTS INTO the structural caps (RECOMMENDED_LIMITS: max_depth=1000 / max_attributes=256 /
# max_bytes=100 MiB) — it parses UNTRUSTED files, so it wants the depth/size refusal defusedxml never
# had (a catchable LimitExceeded ⊂ ValueError → the field degrades, never crashes). `xml_fromstring`
# applies the limits on the purexml path; the stdlib fallback (no purexml) stays UNHARDENED + UN-CAPPED
# (LIMITATIONS documents that gap; `limits=` is never passed to it — stdlib fromstring can't take it).
try:
    from purexml.ElementTree import fromstring as _xml_fromstring_backend
...
# v1.30 (#110): the bare-invocation default output directory name. Used BOTH as the default
...
SCANNER_VERSION = "1.50.0"   # v1.50.0 (MINOR) — public-face pass: (a) the deprecated `scanner` import shim (v1.0.1) is REMOVED — the generic top-level name `scanner` no longer occupies every install; the import path is outside the manifest contract (v1.0.1 precedent), so this is a minor, not a major; (b) `py.typed` shipped so the `Typing :: Typed` classifier is true; (c) README links absolute (48 relative links 404'd on the PyPI page); (d) LICENSE is the verbatim AGPL text (GitHub/scanners detect it; the dual-license terms move to LICENSING.md); SPDX `license` + `license-files` in pyproject; (e) CI matrix gains 3.13/3.14 (+ classifier); (f) sdist no longer ships tests without their fixtures; (g) `python -m file_observer`; (h) pytest>=9 `importorskip` only catches ModuleNotFoundError → the MCP tests now SKIP (not error) without the [mcp] extra. Manifest byte-identical → LOGIC 1.26.0 / SCHEMA 1.25 FROZEN. Prior v1.49.0 (MINOR) — chatlog JSON floor made CONDITIONAL on content shape: a TWO-object session detects when the roles are recognised conversational VALUES and every turn is utterance-shaped (LongMemEval: 0/7 at two turns, 100% at >=3; recall 5.9% miss at 19,195 files). STRICTLY ADDITIVE (is_chatlog False->True only, the v1.29 precedent; corpora verified gained-0/lost-0). Two further RFC changes were FALSIFIED at implementation and reverted rather than shipped with their tests weakened (structure-rule utterance gate rejected real terse transcripts — `hi`/`hello` are non-utterances exactly like `Linux`/`Draft`; alternation-instead-of-recurrence reopened the v1.2.2 i18n FP) — see docs/v1.49.0_RFC_Specification.md §3.1/§3.2, kept as documented residuals with their measurements. LOGIC 1.25.0->1.26.0, SCHEMA 1.25 FROZEN, chatlog mv 11->12. Prior v1.47.1 (patch, HISTORY-only) — polish pass: static-analysis + doc-consistency hygiene, ALL behavior byte-identical (manifest unchanged → LOGIC 1.24.6 / SCHEMA 1.24 FROZEN). (a) code hygiene: dead `platform` import + dead locals + ~48 unused test imports + f-string-no-placeholder nits + an undefined `Any` annotation removed. (b) doc-accuracy: PUBLIC_CONTRACT §2.4 now marks presentation/audio STABLE (the v1.47.0 promotion updated §3 but not §2.4) + adds the `ai_session` namespace + chatlog session axes to the provisional list (were provisional-in-code but undocumented) + a v1.31.0→v1.23.0 typo fix; SECURITY supported-versions refreshed; README Action-pin. (c) OPERATIONALIZED GUARDS so the classes can't recur: ruff (F401/F811/F821/F841/F541) in [dev]+CI; a §2.4↔PROVISIONAL_SPECIALIST_FIELDS stability-consistency guard; namespace-doc-completeness + SECURITY-version + SCHEMA-quick-ref guards. (d) killed the version-pin churn: removed 16 redundant per-release test_version_axes → ONE net-current gate. SCANNER 1.47.0→1.47.1. Prior v1.47.0 (MINOR) — promotion pass: the `presentation` (v1.24: slide_count/title/author/application) + `audio` (v1.25: format/bitrate/duration_s/title/artist/album/year) namespaces PROMOTED provisional→stable. The third promotion pass (after v1.10/v1.14/v1.23/v1.31) + exactly the pair v1.31.0 §6 deferred as "too young, season next pass" — now ~3 weeks seasoned (the v1.31 bar), extraction unchanged since ship, corpus-swept + hardened. DESIGNATION-ONLY: manifest byte-identical (stability lives only in `--schema`) → LOGIC 1.24.6 FROZEN; SCHEMA 1.23→1.24 (a promotion is a contract change even with no value change, the v0.11/v1.10/v1.14/v1.23/v1.31 precedent — a consumer keying on `stability` sees these move provisional→stable). HELD: the chatlog family (alpha-locked + the parked non-count redesign), fact_block/ai_session/lexicon_match (too young — the data-gathering tier), format_signatures/is_polyglot (held-by-design, §2.4). `docs/manifest.schema.json` + `docs/SCHEMA.md` regenerated. SCANNER 1.46.8→1.47.0. RFC `docs/v1.47.0_RFC_Specification.md`; `test_v1_47.py`. Prior v1.46.8 (patch, HISTORY-only) — Windows reparse-prune gates on the NAME-SURROGATE tag bit, not the reparse BIT (#169, validated on native NTFS by Tailnet): `_should_prune_dir` pruned any dir with FILE_ATTRIBUTE_REPARSE_POINT set — a SUPERSET that also flags OneDrive Files-On-Demand / Data-Dedup / AppExecLink dirs → their whole in-tree subtree was SILENTLY DROPPED from files[] with no ErrorRecord (a silent-omission on a plausible real Windows config). Now prunes ONLY name surrogates (junctions + dir symlinks — st_reparse_tag & 0x20000000, the documented IsReparseTagNameSurrogate predicate; the IO_REPARSE_TAG_* constants aren't in stdlib, and the surrogate bit future-proofs vs any new class without hard-coding tags) → non-surrogate reparse dirs now DESCEND (the file-level resolve()-containment still catches any real escape). Windows-only file-SET change (a previously-dropped in-tree subtree now appears); Linux byte-identical (_should_prune_dir is only called under `elif _WINDOWS`) → LOGIC 1.24.5→1.24.6 (the v1.46.0/v1.15 cross-platform-LOGIC precedent: observing logic changed, Linux bytes unchanged, so LOGIC bumps for the fingerprint), SCHEMA 1.23 FROZEN, SCANNER 1.46.7→1.46.8. `test_v1_46_8.py`. Prior v1.46.7 (patch, HISTORY-only) — MIME synonym classes: analyze_mime's content-vs-extension check was a strict `detected == extension_mime`, so a legitimate synonym (JSON family application/jsonl ≡ application/x-ndjson ≡ application/json; also xml/rtf/markdown/python pairs) was reported as a MISMATCH → the file scored "degraded". Surfaced by a real jsonl chatlog corpus (recall's LongMemEval synthesis) scoring 0% clean — 19,195/19,195 false mismatches, ALL JSON-family synonyms (libmagic types `.jsonl` as application/x-ndjson multi-line / application/json single-line while the extension map registers application/jsonl). `_mime_labels_compatible` folds SAME-format-different-label classes so a mismatch means a GENUINE content-vs-extension disagreement (a `.pdf` that is really a zip), not two spellings. VALUES-MOVE: matches_extension False→True on synonym-labelled files → manifest_checksum moves for those corpora ONLY (the v1.25.1 accuracy-fix precedent; default corpora byte-identical). SUPERTYPE relationships (text/plain for a text-based format like markdown/yaml — libmagic can't sub-type) deliberately left untouched — a separate, broader question. LOGIC 1.24.4→1.24.5, SCHEMA 1.23 FROZEN, SCANNER 1.46.6→1.46.7. `test_v1_46_7.py`. Prior v1.46.6 (patch, HISTORY-only) — confine lexicon-index members to the index dir (bestiary soak ③ note): a `--lexicon-index` subscription file's `sources` could `../`-escape (or use an absolute path / a symlink) OUT of the index dir before failing loud — the exact "traversal surface" the v1.43 RFC deferred `!#include` to avoid + claimed absent. resolve()-containment in _read_lexicon_index now blocks all three (the read never happens; fails loud with an actionable message). Faithful to the v1.43 self-contained-bundle model; cross-bundle composition stays via separate `--lexicon PATH` flags. Operator-trust so not a strict vuln, but it matters under the distribution/subscription model (a third-party index shouldn't reach outside its bundle). A LOADER hardening — an equivalent VALID (in-bundle) index resolves identically → manifest byte-identical → LOGIC 1.24.4 / SCHEMA 1.23 FROZEN (the v1.43 loader-frozen precedent; only an ESCAPING index changes, from read-then-fail-loud to block-then-fail-loud). Prior 1.46.5 (patch, HISTORY-only) — never-crash: a numeric-typed EXIF make/model no longer aborts the scan (bestiary soak #165, decorrelated capture-metadata leg). A make/model tag with a NUMERIC TIFF type (SHORT/LONG, not ASCII) made fo store an int → _build_summary's .strip() raised AttributeError at SCAN-ABORT scope (corpus-level: 1 bad .jpg crashed a whole dir, no per-file ErrorRecord). Two layers: ROOT CAUSE — _parse_exif_tiff type-gates (make/model/datetime are ASCII by spec → a numeric-typed tag is None, never an int; orientation stays the lone numeric SHORT); DEFENSE — _build_summary only lets STRING make/model contribute to a device label (the aggregate path can't abort on a non-string; the other aggregate string sites — author_aggregate, provenance — already had the isinstance guard, so the summary make/model site was the lone gap). Value changes ONLY on a malformed EXIF file (int→None) → manifest_checksum moves for those only (the v1.8.2/v1.25.1 degraded-path precedent; real corpora → NO-DRIFT). LOGIC 1.24.3→1.24.4, SCHEMA 1.23 FROZEN. Prior 1.46.4 (patch, HISTORY-only) — recognize macOS-universal metadata (.DS_Store=Bud1, ._*=AppleDouble) by magic so they stop spraying unsupported_extension error-records on every Mac corpus scan (MacBook Neo APFS shakedown finding). Two MAGIC_SIGNATURES entries + a recognition-gate arm: a magic match IS a positive content ID, so they count as identified even when libmagic types them octet-stream (a genuine unknown like a random binary still flags — control-tested). RECOGNITION change (v1.22/v1.23.3 precedent): unsupported_extension flips False + format_signatures gains the label + error_files drops for these files → manifest_checksum moves for .DS_Store/AppleDouble-bearing manifests (no new field; corpora without them → sweep NO-DRIFT). LOGIC 1.24.2→1.24.3, SCHEMA 1.23 FROZEN. Prior 1.46.3 (patch, HISTORY-only) — MCP hardening from bestiary's consumer-seat shakedown (2 filed): #161 scan_file now OBSERVES `sidecar_exists` on the ORIGINAL path (it's a neighbourhood stat via fo's own detect_sidecar, not a sibling scan — so the isolated copy's False was a silent-wrong; re-observed → correct, matching scan_directory; asset_matches is content-derived → unaffected, over-broad in-code comment corrected); #162 the pre-scan work guard's `max_files` is now clamped to a server ceiling (`SERVER_MAX_FILES`=100_000, floor 1) — tool args are LLM-constructed, so an over-set max_files (e.g. 10M) previously defeated the guard (unbounded walk+read+hash); floor also fixes a negative max_files "more than -1 files" message. MCP-SURFACE ONLY — the manifest is UNCHANGED (scan_directory still reports sidecar_exists normally; scan_file's null is a front-door projection) → LOGIC 1.24.2 / SCHEMA 1.23 FROZEN (the v1.38.1 MCP/CLI-runtime patch precedent). Prior 1.46.2 (patch, HISTORY-only) — pin .vx → text/plain: the Windows shakedown MIME sweep (v2) found .vx flips is_binary=True on the no-libmagic Windows path (guess_type returns None → application/octet-stream, a BINARY type), while Linux libmagic reads it text/plain=False — the same cross-OS is_binary-flip class as the v1.46 .csv pin. Pinning text/plain aligns every OS (keeps matches_extension True vs libmagic; the .toml/.csv precedent). Changes .vx extension_mime None→text/plain + is_binary on the no-libmagic path → manifest_checksum moves for .vx-bearing manifests (values-move; no new field, no routing flip; no .vx in any corpus → sweep NO-DRIFT). LOGIC 1.24.1→1.24.2, SCHEMA 1.23 FROZEN. Prior 1.46.1 (#156, patch, HISTORY-only) — add the missing signal_provenance["structural.csv_headers"] entry (mirrors structural.title/document_keys; new trigger csv_header_row). Pre-existing gap surfaced in the v1.46 review: csv_headers was extracted with NO provenance entry while its structural siblings had one. Adds a provenance key on CSV-with-headers files → manifest_checksum MOVES for CSV-bearing manifests (the v1.8.2/v1.9.1/v1.25.1 values-move / provenance-accuracy patch precedent); no new manifest FIELD, no routing flag flip. LOGIC 1.24.0→1.24.1, SCHEMA 1.23 FROZEN. Prior 1.46.0 — native-Windows/NTFS hardening (Windows shakedown findings): (A) reparse-aware discovery containment — iter_files now walks with os.walk and PRUNES reparse-point dirs (NTFS junctions/dir symlinks) on Windows BEFORE descending, so a crafted out-of-tree or cyclic junction can't escape the tree / double-count / hang the scan (rglob materialized the junction's whole subtree first — leg-4/Codex P1); fail-closed via resolve()-containment on an inconclusive attribute read (leg-4/CodeRabbit); POSIX is byte-identical (os.walk followlinks=False omits symlinked dirs exactly as rglob did — validated on a hostile nested/hidden/symlink tree); the v1.8.1 file-symlink containment is retained (leg-1 refined A away from an unconditional resolve()-for-every-file, which would silently drop a mid-scan-deleted file); (B) pin .csv→text/csv so a plain CSV isn't typed BINARY (is_binary flip) via the Windows MIME registry's application/vnd.ms-excel on the no-libmagic path — restores csv_headers; (C) byte-safe --watch/--schema stdout (route through stdout.buffer UTF-8 like the main --stdout so a non-ASCII payload doesn't crash a cp1252 console). A+B are cross-platform routing-LOGIC changes (Linux real-corpus output BYTE-IDENTICAL → sweep NO-DRIFT; new behavior Windows-only) → LOGIC 1.23.0→1.24.0; C is output-routing only. SCHEMA 1.23 FROZEN. SCANNER 1.45.0→1.46.0. Prior 1.45.0 — decouple the human summary from the determinism contract + refresh it (#147): the per-scan `summary` is a DERIVED VIEW of already-checksummed data (files/vectors/quality), yet it sat INSIDE manifest_checksum, so every prose refresh cost a LOGIC bump (v1.19 did; #147 would). v1.45 excludes the WHOLE `summary` from the checksum (joins scan_id/generated_at) — a ONE-TIME ungated LOGIC bump (every manifest's checksum moves once; the v1.15/v1.36 every-checksum-moves precedent), after which summary refreshes are LOGIC-FREE forever. Decision A (approved) over a non-indexed sidecar: a sidecar would FRAGMENT the human summary, and readability (one paragraph, one glance) is its whole purpose — so the summary stays ONE coherent readable field, just no longer sealed. Consequence (documented, PUBLIC_CONTRACT §1.8/§1.9): the manifest_signature is computed FROM the checksum, so a summary outside the checksum is also OUTSIDE signature coverage → the summary is a non-sealed, unsigned, build-versioned derived view; trust the sealed machine fields (it's reconstructible from them). Rides FREE in the same release: the #147 refresh — `_build_summary` now surfaces the lexicon vector shape (files_matched + K categories + hits; the lexicon_match xN flag count already showed), a `Fact blocks: N files` line (v1.32 is_fact_block), and an `AI sessions: N logs, T tokens observed (never priced)` line (v1.33/v1.35 ai_session usage sums) — each GATED on presence so a corpus without them renders an unchanged summary. ai_session tokens are COUNTED, never priced (the federation charter). SCHEMA 1.23 FROZEN (the summary field's shape is unchanged — only its role in the checksum). SCANNER 1.44.0→1.45.0, LOGIC 1.22.0→1.23.0. Prior 1.44.0 — lexicon category breakdown survives `--trusted-only` (#146): safe mode (v1.40) is the mode a consumer uses to feed a manifest to a model, and the bring-your-own lexicon (v1.38) is the guardrail pre-screen whose whole point is safe consumption — but the two together silently DROPPED the per-category breakdown a consumer routes on. The projection nulls every string leaf AND drops every dict KEY not in a static fo-known allowlist (a dict key can be attacker-controlled — chatlog speaker_turn_counts is keyed by the speaker LABEL); lexicon category names are dynamic consumer-config dict keys, indistinguishable from an attacker label by that generic rule → dropped. The finding that makes it safe to fix: the ENTIRE lexicon_match namespace + the lexicon vector summary are counts (fo_derived) keyed by category names / a lexicon_id that are CONSUMER CONFIGURATION — never bytes from the scanned file (v1.38's terms-never-emitted invariant). v1.44 names a third trust class `consumer_config` (a string that originates from the consumer's own runtime config, never a scanned file → trusted in safe mode) and keeps the lexicon per-category signal via a SCOPED trusted-subtree relaxation inside lexicon_match ONLY: category-name keys kept (the `categories` sub-dict), `lexicon_id` kept (the one allowed string), counts/densities kept; EVERY other leaf falls through to the generic nulling projector (safe-by-construction against a future file-derived field) + a completeness guard (test_lexicon_match_trust_complete) fails if a new lexicon_match field appears unclassified. The corpus `lexicon` vector summary is likewise kept (other vector summaries stay emptied — they're laced with file-derived author/producer names). A PROJECTION change → the DEFAULT manifest is byte-identical, so LOGIC 1.22.0 / SCHEMA 1.23 FROZEN (the v1.40 --trusted-only precedent); the --schema field-level trust annotation is UNCHANGED (specialist_metadata stays TRUST_MIXED — it doesn't annotate nested leaves), so schema_doc_version does NOT bump. SCANNER 1.43.0→1.44.0. THE canary test proves a distinctive attacker-marker in a file's body never rides along into the trusted output. Prior 1.43.0 — lexicon source loader (distribution format + composition): the bring-your-own-lexicon observer (v1.38/v1.41) loaded exactly ONE flat JSON file; v1.43 lets a consumer SOURCE a lexicon the way risk/blocklist term lists are actually distributed (uBlock/EasyList/hosts). Two accepted formats normalize to the SAME internal {lexicon_id, categories:{cat:[terms]}}: JSON (canonical, extended with OPTIONAL load-time-only header keys version/source/license/description) + an EasyList-style TEXT format (`! Title:`/`! Version:` header, `[category]` sections, one-term-per-line, `!`/`#` comments; literal terms only → ReDoS-safe). Composition: repeatable `--lexicon` flags (union + dedup, ORDER-INDEPENDENT) + a `--lexicon-index` subscription file (JSON {sources:[...]} or a text path list, members resolved relative to the index dir); `--lexicon-id` overrides the composed id. `!#include` deferred (traversal/cycle surface). Source PROVENANCE stays load-time-only (stderr summary) and NEVER enters the manifest — a list's title can itself signal intent, so only the composed lexicon_id + dictionary_id cross, exactly as v1.38-v1.42. dictionary_id stays CONTENT-anchored (a version bump with an identical term set → the same id; a term change moves it); a declared header `count` that disagrees with the parsed total → a stderr WARNING, never a failure; `expires` never enforced (no clock behavior — determinism). fo composes LOCAL files; it never fetches (the offline-deterministic charter — distribution's fetch/update half stays a subscription tool's job). A LOADER upgrade: the resolved lexicon + dictionary_id are behavior-identical for an equivalent term set → a scan resolving to the same terms is byte-identical with the same dictionary_id; the existing single flat JSON loads identically → LOGIC/SCHEMA FROZEN (1.22.0/1.23), SCANNER 1.42.0→1.43.0 (the v1.26/v1.39 loader-ergonomics/front-door precedent). The new text/index parsers earn the v1.8.1 bounded/never-crash/values-neutral bar (LEXICON_MAX_SOURCES/MAX_LINE_LEN caps, strict UTF-8, fail-loud). Prior 1.42.0 — screening receipt + bridge id (`--receipt`, #133 Phase 3): a compact audit-record PROJECTION of the built manifest (envelope: versions/manifest_checksum/signature/scan_id/dictionary_id + a per-file receipt: receipt_id/path_id/checksum/size/mime/safety_flags/lexicon-hit-summary). The `receipt_id` is a tamper-evident sha256 over a length-prefixed preimage of (manifest_checksum + rel_path + file-hash) — the explicit join key a downstream read/skip log references (blah_mad's bridge). A projection runs AFTER manifest_checksum exists (no circularity) → LOGIC/SCHEMA FROZEN (1.22.0/1.23), default manifest byte-identical (the v1.26/v1.28/v1.37/v1.40 front-door precedent); new `receipt_doc_version` versions the envelope. Safe by construction (no raw path — path_id correlates). fo NEVER records the read/skip decision (the charter boundary). Prior 1.41.0 — lexicon self-sweep over file-derived metadata (#136 detection half, Phase 2 of the r/mcp consumption-safety plan). The v1.38 lexicon counts terms in a file's BODY text; a risk term can hide where the body scan structurally never looks — a filename, an EXIF make/model, a PDF producer/title/author, an office application string. v1.41 reuses the pure `_lexicon_scan` matcher over the COLLECTED file-derived metadata strings (filename [not the full path] + tags + frontmatter + structural + the string leaves in specialist_metadata, minus the fo-generated lexicon_match namespace; content_preview EXCLUDED — body-covered) and reports the result in a new provisional `lexicon_match.metadata` sub-block ({categories:{cat:{count,density}}, total_hits, total_tokens}). Binary files (images/PDFs/office) get NO body scan but DO have metadata hits, so the sweep CREATES the lexicon_match namespace (with a zero body block) when it isn't already present, keeping the body+metadata shape consistent. The existing `lexicon_match` safety_flag now fires on a body OR metadata hit (a hit is a hit; presence, not verdict). Metadata text capped at LEXICON_METADATA_MAX_CHARS (a hostile PDF could carry a huge producer). Dormant without a lexicon → existing manifests byte-identical (the v1.38/v1.30 gated pattern). Measure-first (307 real files, benign lexicon): 19% metadata-ONLY hits — the coverage gap this fills. LOGIC 1.21.0→1.22.0 (values-move only on lexicon scans), SCHEMA 1.22→1.23 (additive provisional field), lexicon method_version 1→2. Prior 1.40.0 — field trust classification + `--trusted-only` safe-mode projection + `--schema` trust annotation (#136). A first-class FIELD_TRUST registry tags every emitted field fo_derived (trusted — numbers/booleans/hashes/enums/flags, can't carry a text payload) vs file_derived (untrusted — verbatim free-text bytes from the input: path/filename/content_preview/tags/frontmatter/metadata strings, which an attacker controls and which ride into a downstream model). `--trusted-only` emits a PROJECTION that nulls the file_derived fields + adds a top-level `trusted_only: true` marker and a fo-derived `path_id = sha256(rel-posix path)` correlation handle — safe by construction for the guardrail-screen use case (the model only ever sees fo-generated signal). A projection, no new observation, no sanitization → the DEFAULT manifest is byte-identical, LOGIC/SCHEMA UNCHANGED (the v1.26/v1.28/v1.37 front-door precedent); `schema_doc_version` bumps for the `--schema` trust annotation. Prior 1.39.0 — MCP front-door gains lexicon + delta: the v1.37 MCP server predated v1.38 lexicon, so the agent front-door couldn't run the guardrail pre-screen. v1.39 threads the v1.38 lexicon (server-startup `--lexicon` flag — terms never cross the MCP wire) + delta scanning (`previous_manifest_path` tool param) through the existing 4 tools. A front-door: manifest byte-identical to a CLI scan with the same settings → LOGIC/SCHEMA UNCHANGED (the v1.26/v1.28/v1.37 precedent). Prior 1.38.1 (#126, patch, HISTORY-only) — add a `--version`/`-V` CLI flag (prints `file-observer X.Y.Z`, exit 0); un-breaks downstream version probing (recall doctor read the usage banner → false "below floor"). Runtime-only output, manifest UNCHANGED (the --workers/--watch precedent) → LOGIC/SCHEMA frozen. Prior 1.38.0 — bring-your-own-lexicon term observer: fo counts a CONSUMER-SUPPLIED, category-tagged lexicon's terms in a file's text and reports per-category counts + density (an OBSERVATION, never a verdict — the consumer thresholds). Values-neutral engine (built/tested with benign placeholder terms only); the sensitive lexicon is runtime config, never committed, never echoed into the manifest (only counts + category names + a content-hash dictionary_id). Motivated by knowing a file is a guardrail-trip risk BEFORE handing it to an AI (fo is not an AI → safe to read what an LLM couldn't). Word-boundary token(-sequence) match (measure-first: substring over-counts 9.4x on benign terms); full-file bounded read (64 MiB; a risk term can sit anywhere in a long log); gated on a lexicon → dormant otherwise (byte-identical). New provisional `lexicon_match` specialist namespace + `lexicon_match` safety_flag + `lexicon` vector (dictionary_id = sha256 of lexicon_id+sorted terms → dual-falsification). Prior 1.37.0 = MCP server (agent-native front-door). (`--receipt`, #133 Phase 3): a compact audit-record PROJECTION of the built manifest (envelope: versions/manifest_checksum/signature/scan_id/dictionary_id + a per-file receipt: receipt_id/path_id/checksum/size/mime/safety_flags/lexicon-hit-summary). The `receipt_id` is a tamper-evident sha256 over a length-prefixed preimage of (manifest_checksum + rel_path + file-hash) — the explicit join key a downstream read/skip log references (blah_mad's bridge). A projection runs AFTER manifest_checksum exists (no circularity) → LOGIC/SCHEMA FROZEN (1.22.0/1.23), default manifest byte-identical (the v1.26/v1.28/v1.37/v1.40 front-door precedent); new `receipt_doc_version` versions the envelope. Safe by construction (no raw path — path_id correlates). fo NEVER records the read/skip decision (the charter boundary). Prior 1.41.0 — lexicon self-sweep over file-derived metadata (#136 detection half, Phase 2 of the r/mcp consumption-safety plan). The v1.38 lexicon counts terms in a file's BODY text; a risk term can hide where the body scan structurally never looks — a filename, an EXIF make/model, a PDF producer/title/author, an office application string. v1.41 reuses the pure `_lexicon_scan` matcher over the COLLECTED file-derived metadata strings (filename [not the full path] + tags + frontmatter + structural + the string leaves in specialist_metadata, minus the fo-generated lexicon_match namespace; content_preview EXCLUDED — body-covered) and reports the result in a new provisional `lexicon_match.metadata` sub-block ({categories:{cat:{count,density}}, total_hits, total_tokens}). Binary files (images/PDFs/office) get NO body scan but DO have metadata hits, so the sweep CREATES the lexicon_match namespace (with a zero body block) when it isn't already present, keeping the body+metadata shape consistent. The existing `lexicon_match` safety_flag now fires on a body OR metadata hit (a hit is a hit; presence, not verdict). Metadata text capped at LEXICON_METADATA_MAX_CHARS (a hostile PDF could carry a huge producer). Dormant without a lexicon → existing manifests byte-identical (the v1.38/v1.30 gated pattern). Measure-first (307 real files, benign lexicon): 19% metadata-ONLY hits — the coverage gap this fills. LOGIC 1.21.0→1.22.0 (values-move only on lexicon scans), SCHEMA 1.22→1.23 (additive provisional field), lexicon method_version 1→2. Prior 1.40.0 — field trust classification + `--trusted-only` safe-mode projection + `--schema` trust annotation (#136). A first-class FIELD_TRUST registry tags every emitted field fo_derived (trusted — numbers/booleans/hashes/enums/flags, can't carry a text payload) vs file_derived (untrusted — verbatim free-text bytes from the input: path/filename/content_preview/tags/frontmatter/metadata strings, which an attacker controls and which ride into a downstream model). `--trusted-only` emits a PROJECTION that nulls the file_derived fields + adds a top-level `trusted_only: true` marker and a fo-derived `path_id = sha256(rel-posix path)` correlation handle — safe by construction for the guardrail-screen use case (the model only ever sees fo-generated signal). A projection, no new observation, no sanitization → the DEFAULT manifest is byte-identical, LOGIC/SCHEMA UNCHANGED (the v1.26/v1.28/v1.37 front-door precedent); `schema_doc_version` bumps for the `--schema` trust annotation. Prior 1.39.0 — MCP front-door gains lexicon + delta: the v1.37 MCP server predated v1.38 lexicon, so the agent front-door couldn't run the guardrail pre-screen. v1.39 threads the v1.38 lexicon (server-startup `--lexicon` flag — terms never cross the MCP wire) + delta scanning (`previous_manifest_path` tool param) through the existing 4 tools. A front-door: manifest byte-identical to a CLI scan with the same settings → LOGIC/SCHEMA UNCHANGED (the v1.26/v1.28/v1.37 precedent). Prior 1.38.1 (#126, patch, HISTORY-only) — add a `--version`/`-V` CLI flag (prints `file-observer X.Y.Z`, exit 0); un-breaks downstream version probing (recall doctor read the usage banner → false "below floor"). Runtime-only output, manifest UNCHANGED (the --workers/--watch precedent) → LOGIC/SCHEMA frozen. Prior 1.38.0 — bring-your-own-lexicon term observer: fo counts a CONSUMER-SUPPLIED, category-tagged lexicon's terms in a file's text and reports per-category counts + density (an OBSERVATION, never a verdict — the consumer thresholds). Values-neutral engine (built/tested with benign placeholder terms only); the sensitive lexicon is runtime config, never committed, never echoed into the manifest (only counts + category names + a content-hash dictionary_id). Motivated by knowing a file is a guardrail-trip risk BEFORE handing it to an AI (fo is not an AI → safe to read what an LLM couldn't). Word-boundary token(-sequence) match (measure-first: substring over-counts 9.4x on benign terms); full-file bounded read (64 MiB; a risk term can sit anywhere in a long log); gated on a lexicon → dormant otherwise (byte-identical). New provisional `lexicon_match` specialist namespace + `lexicon_match` safety_flag + `lexicon` vector (dictionary_id = sha256 of lexicon_id+sorted terms → dual-falsification). Prior 1.37.0 = MCP server (agent-native front-door).
LOGIC_VERSION = "1.26.0"   # v1.46.8 — Windows reparse-prune gates on the NAME-SURROGATE tag bit (0x20000000), not the reparse BIT (#169): a non-surrogate reparse dir (OneDrive/Data-Dedup/AppExecLink) is now DESCENDED instead of pruned → its in-tree subtree reappears in files[] on Windows; junctions/dir-symlinks still prune (containment intact, Tailnet-validated on native NTFS). Windows-only file-SET change; Linux byte-identical (_should_prune_dir runs only under `elif _WINDOWS`) → LOGIC bumps for the fingerprint (the v1.46.0/v1.15 cross-platform-LOGIC precedent). Prior 1.24.5 = v1.46.7 — MIME synonym classes (_mime_labels_compatible): analyze_mime folds SAME-format-different-label pairs (JSON family application/jsonl ≡ application/x-ndjson ≡ application/json + xml/rtf/markdown/python pairs) so matches_extension flags only a GENUINE content-vs-extension disagreement — False→True on synonym-labelled files → manifest_checksum moves for those corpora ONLY (the v1.25.1 accuracy-fix precedent; default corpora byte-identical; SUPERTYPE relationships like text/plain-for-markdown/yaml deliberately left untouched — a separate broader question). Prior 1.24.4 = v1.46.5 — never-crash EXIF type-gate (bestiary #165): a numeric-typed make/model is now None (str|None invariant), not an int → manifest_checksum moves ONLY for a malformed-EXIF file (int→None; the v1.8.2/v1.25.1 degraded-path precedent; real corpora NO-DRIFT). Prior 1.24.3 = v1.46.4 — recognize .DS_Store (Bud1) / AppleDouble (._*) macOS metadata by magic: unsupported_extension flips False + format_signatures gains the label for these files → manifest_checksum moves for .DS_Store/AppleDouble-bearing manifests only (the v1.22/v1.23.3 recognition-change precedent; corpora without them → NO-DRIFT). Prior 1.24.2 = v1.46.2 — pin .vx → text/plain (Windows shakedown MIME-sweep finding): .vx no longer flips is_binary on the no-libmagic Windows path; extension_mime None→text/plain + is_binary/matches_extension move for .vx-bearing manifests (a values-move; the v1.46 .csv-pin class; no .vx in any corpus → sweep NO-DRIFT). Prior 1.24.1 = v1.46.1 (#156) — csv_headers signal_provenance entry now emitted (new trigger csv_header_row): manifest_checksum moves for CSV-with-headers manifests (a values-move — a new provenance key appears; no new manifest field, no routing flag flip; the v1.25.1 provenance-accuracy-patch precedent). Prior 1.24.0 = v1.46.0 — native-Windows/NTFS hardening (A+B): iter_files reparse-aware containment (os.walk prunes reparse-point dirs before descent, Windows-only; the v1.8.1 file-symlink containment retained) + .csv pinned to text/csv. Both are cross-platform routing-LOGIC changes: Linux real-corpus output is BYTE-IDENTICAL (A skips only out-of-tree-resolving files, which v1.8.1 already skipped for file symlinks; B is a no-op on Linux — .csv is already text/csv there) → sweep NO-DRIFT; the new behavior is Windows-only (junction escape closed; CSV no longer mistyped binary). The v1.15 cross-platform-LOGIC precedent — the observing logic changed even though Linux bytes are unchanged, so LOGIC bumps for the fingerprint. Prior 1.23.0 = v1.45.0 (#147) — ONE-TIME: the checksum computation changed — the human-readable `summary` is now EXCLUDED from manifest_checksum (it's a derived view, not an observation), so EVERY manifest's checksum moves once (ungated, the v1.15 ScanContext / v1.36 purexml-dep-record every-checksum-moves precedent). After this, summary refreshes are LOGIC-free. The #147 summary content refresh itself moves nothing (summary is now excluded). Prior 1.22.0 = v1.41.0 — lexicon self-sweep over file-derived metadata: new derived values (the lexicon_match.metadata sub-block) move manifest_checksum ONLY on lexicon scans (the v1.29/v1.33 values-move + v1.38 gated precedent); no routing flag flips; no-lexicon scans byte-identical. Prior 1.21.0 = v1.38.0 — bring-your-own-lexicon term observer: a new baseline-tier content derivation (per-category term counts + density on a consumer-supplied lexicon). manifest_checksum moves ONLY for lexicon-supplied scans (the v1.30 gated-feature precedent) — no-lexicon scans are byte-identical, no routing flag flips. Prior 1.20.0 = v1.36.0 — defusedxml→purexml XML-dependency changeover: fo's XML hardening (OOXML/ODF specialists + structural XML-keys tier) moves from defusedxml to purexml (pure-stdlib, zero-dep, oracle-gated defusedxml replacement, MIT; capability-proven 2695/0 on fo's own corpus + 467/0 purexml's). fo OPTS INTO purexml's structural caps (RECOMMENDED_LIMITS: max_depth=1000/max_attributes=256/max_bytes=100 MiB) via `xml_fromstring` on the purexml path — so fo now REJECTS a pathologically-deep/attribute-flooded/oversized XML that defusedxml dutifully parsed (a catchable LimitExceeded⊂ValueError → the field degrades to null/error, never a crash). PARSE OUTPUT BYTE-IDENTICAL on real files (2695/0); output changes ONLY on pathological XML inputs → LOGIC bump (the v1.30.2 bounded-observation "output changes only on pathological input" precedent); no routing flag flips. Separately, ScanContext.dependencies records purexml (name+version+the applied limits) in place of defusedxml → manifest_checksum moves for EVERY manifest (the dep-record is in every ScanContext — Pillar 1: ScanContext explains environment variance; the same as any dep version bump). The stdlib fallback (no-purexml) stays unhardened + un-capped (LIMITATIONS). Prior 1.19.0 = v1.35.0 — AI-session per-model usage attribution (increment 3): a new provisional `ai_session.usage_by_model` — a deterministic list (real models sorted, null bucket last) of per-model token-usage SUMS, keyed on the model CO-LOCATED with each usage dict (Claude Code message.model / OpenAI response.model / Gemini modelVersion — all measured 100% co-located). INVARIANT: `usage` == elementwise sum of `usage_by_model` (the tested v1.33 session path untouched → session `usage` byte-identical). Model verbatim incl. markers (`<synthetic>`); model-less usage-turns + a hostile-cardinality overflow beyond AI_SESSION_MAX_MODELS → the null bucket (sums stay complete). NEVER priced (the bright line). ai_session method_version 1→2 (per-model grouping is a new block rule → feeds the ai_session rules_hash). VALUES move manifest_checksum on ai_session corpora (the v1.29/v1.33 values-move precedent → LOGIC bump); no routing flag flips. Prior 1.18.0 = v1.34.0 — chatlog session axes (recall#62): the chatlog specialist emits three new FLAT top-level scalars — first_timestamp/last_timestamp (min/max turn timestamp, recognized-key set {timestamp,created_date,create_time,created_at}, parsed ISO-string-or-epoch → canonical ISO-8601 UTC `…Z`/ms, null-when-untimestamped) + cwd (first-seen top-level cwd, verbatim/bounded, null-when-absent). Deterministic pure function of the file (observe, don't derive); values move manifest_checksum on timestamped/cwd-bearing chatlog corpora (the v1.29/v1.33 values-move precedent → LOGIC bump); no routing flag flips, is_chatlog unchanged. chatlog method_version 10→11 (new signals + recognized-key set/normalization feed the chatlog rules_hash). Prior 1.17.0 = v1.33.0 — AI-session observation increment 1: a new provisional `ai_session` namespace on is_chatlog-detected AI session logs (Claude Code / OpenAI / Gemini), carrying token-usage SUMS (canonical fo names, null-per-absent, vendor raw keys preserved) + a producer-schema fingerprint (vendor/surface/models/id_prefix/object_types/schema_mismatch) anchored on id-prefix+object-type (NOT the usage-key vocab — OpenAI Responses collides with Anthropic on input_tokens; measure_ai_session_2026-07-05). Observe-only: SUMS never priced. VALUES move manifest_checksum on AI-session corpora (the v1.29 values-move precedent → LOGIC bump); no routing flag flips. Prior 1.16.0 = v1.32.0 — new content-detection routing: the generic kv-fact-block specialist (FR #114). A text file whose BODY (frontmatter stripped) is a `key: value` block now carries `is_fact_block` + a `fact_block` specialist dispatch (content-shape, any text body; the sentence-value veto keeps it off dialogue). Additive: `is_fact_block` False→True only; no file's prior routing flips away. The v1.2/v1.29 detection-LOGIC precedent. Prior 1.15.3 = v1.30.2 — ReDoS / bounded-TIME hardening (red-team class the v1.8.1 size/crash/escape pass didn't cover): three content regexes backtracked super-linearly on bounded-size-but-pathological input — CHATLOG_WIKI_LINK_RE (`\[\[.+?\]\]` → ~13 s on 64 KB of `[[`), ASSET_RE (~1.7 s on 64 KB of `[`), PROVENANCE_VERSION_SUFFIX_RE (`\s+…\s*…` overlap → hung on a 64 KB-whitespace PDF /Producer; found by the falsify-first all-regex battery, not manual read). All three bounded/anchored → linear (≤53 ms); a standing guard (test_regex_redos_hardening.py) battery-tests every compiled regex under a hard timeout so the class can't regress. Behavior byte-identical on real content (verified parity: wiki links, markdown assets, 10 real producer strings incl. the leading-whitespace edge); output changes ONLY on bracket-bearing / >400-char-target / >32-whitespace pathological inputs → manifest_checksum moves for those files only (the v1.8.1 red-team-hardening precedent; sweep NO-DRIFT — corpora lack such inputs). Prior 1.15.2 = v1.30.1 — the v1.30.0 self-inclusion skip is ANCHORED to fo's actual RESOLVED output dir (prefix-match on its rel path, normcase) instead of a bare-name match on `file-observer-manifests` at any depth — so an UNRELATED user dir that merely shares the name is no longer silently dropped from the manifest (leg-2/OpenAI red-team: silent data loss; also fixes the case-insensitive-fs miss). Runtime `skip_output_dir` (the CLI's own output dir; excluded from meta.config) drives it; the API never sets it → never skips. `manifest_checksum` moves ONLY for a tree containing an unrelated `file-observer-manifests` dir (now included) — corpora don't, so the sweep stays NO-DRIFT. Prior 1.15.1 = v1.30.0 — discovery SKIPS the tool's own default output dir (`file-observer-manifests/`) so a re-scan of the cwd never observes its own prior manifest/report (the leg-1 self-inclusion catch from the #110 default-output relocation). Prior 1.15.0 = v1.29.0 — chatlog detection recognizes agentic (tool-turn) sessions: a turn counts when it has a conversational role + a text-bearing block (backward-compat) OR a distinctive agentic block (thinking/tool_use/tool_result), in detection AND turn-counting signals (prose signals stay text+thinking only; generic image/document are NOT triggers — leg-1 review FP fix). Recovers tool-heavy Claude Code logs the text-centric gate false-negatived (3/28 real federation logs, incl. a 139MB session); falsify-first-validated FP-clean vs telemetry/RBAC/func-call/gallery/doc-store JSON. Strict superset of v1.28 → is_chatlog additive (False→True, never True→False); chatlog signal VALUES move for agentic logs (method_version 9→10) → manifest_checksum moves on agentic corpora. Prior 1.14.1 = v1.25.1 — OLE2 specialists (.doc/.xls/.msg/.ppt) declare a full-file deviation in signal_provenance (`ole2_full_file_required`) instead of the false `bounded_sample`; provenance-accuracy only, no extracted value changes, but moves manifest_checksum (the v1.8.2/v1.9.1 manifest-surface precedent). leg-4/Codex P2 on PR #98, OLE2-family-wide pre-existing. Prior 1.14.0 = v1.25.0 — audio (.mp3) + legacy presentation (.ppt) extraction (Candidate B ph.2): new `audio` namespace (ID3 + bounded MPEG frame-header parse) + .ppt via OLE2 → requires_specialist_tool flips False→True for both (routing change; the v1.16/v1.24 precedent). Prior 1.13.0 = v1.24.0 — office+image extraction (Candidate B ph.1): new specialists for .pptx/.odp/.odt/.ods/.jp2/.tiff/.tif → requires_specialist_tool flips False→True for them (routing change; the v1.16 precedent). Prior 1.12.4 = v1.23.3 — bzip2 dual-magic + `_OneOf` byte-alternation matcher: recognizes empty/data-less bzip2 (end-of-stream magic at offset 4, not the block magic) while rejecting prose + an invalid level byte; reconciled 0/0 with the puresniff clean-room replica. Prior 1.12.3 = v1.23.2 — corroborated PDF-header sniff: the `%PDF-` MIME-sniff window widens 256->1024 (matching the scanner's own `sample[:1024]` PDF-header tolerance) AND requires a corroborating PDF-structure token (`PDF_STRUCTURE_TOKENS`), so a real junk-prefixed PDF is typed while a deep literal with no structure is rejected; C2/`scan_signatures` stays pure find-anywhere. Prior 1.12.2 = v1.23.1 — PDF-header FP fix (C1/C2 split): the find-anywhere `%PDF-` magic rule is bounded to a 256-byte header window in the MIME sniff (C1, `_sniff_mime`) via the `_Within` sentinel — a stray deep `%PDF-` in a source/text file no longer types it `application/pdf` (no-libmagic path) — while `scan_signatures` (C2, format_signatures/is_polyglot) keeps find-anywhere so a real embedded PDF still registers (is_polyglot stays honest). FP surfaced by puresniff's clean-room sweep, loose since v1.3. Prior 1.12.1 = v1.22.1 — `.eml` MIME-guard relaxation: accept text/plain & text/html for .eml (libmagic types body-dominated mail as text, not message/rfc822, so the email specialist was wrongly skipped); extension-gated so a lying text `.msg` stays distrusted. Same class as v1.15.2. Prior 1.12.0 = v1.22.0 — content-aware recognition extended to BINARY: unsupported_extension fires ONLY when content didn't identify the file (octet-stream / extension-fallback / unreadable), NOT when identified-but-no-specialist. Recognition-only, no new extraction. supported counter now single-source (not-flagged AND not-stat-failed). Prior 1.11.0 = v1.21.0 — content-aware recognition (Option B) for TEXT: same diagnostic, text-only (text/* or known text-app MIME); supported/unsupported counters shifted. Prior 1.10.0 = v1.20.0 — video.creation_date_qt (Apple QuickTime creationdate key, capture moment WITH timezone, separate from mvhd creation_date — observe-don't-reconcile). Prior 1.9.0 = v1.19.0 — human-readable summary refresh: _build_summary surfaces provenance/capture-metadata/named-safety-flags/preservation + comments on ambiguity (the summary string feeds manifest_checksum). + new --schema --format summary (prose self-description, separate surface). Prior 1.8.0 — video capture device + GPS-presence: make/model (Apple QuickTime keys via moov→meta→keys/ilst) + gps_present/gps_source (location.ISO6709, presence not coordinates) → geotagged fires for video. New extraction + safety_flag routing. Prior 1.7.0 = v1.17.0 video container half.
SCHEMA_VERSION = "1.25"   # v1.47.0 — promotion pass: the `presentation` (slide_count/title/author/application) + `audio` (format/bitrate/duration_s/title/artist/album/year) namespaces promoted provisional→stable. A promotion is a contract change even with no value change (the v0.11/v1.10/v1.14/v1.23/v1.31 precedent); designation-only, manifest byte-identical (stability lives only in `--schema`). Prior 1.23 = v1.41.0 — additive: new provisional `lexicon_match.metadata` sub-block ({categories/total_hits/total_tokens} over the file-derived metadata self-sweep); a new field in an existing namespace = additive contract change (the v1.20/v1.33/v1.35 precedent), provisional. Prior 1.22 = v1.38.0 — additive: new provisional `lexicon_match` specialist namespace (lexicon_id/categories/total_hits) + new `lexicon_match` safety_flag + new `lexicon` vector + new `lexicon_full_file` provenance trigger, for the bring-your-own-lexicon term observer. A new namespace/vector/flag = contract-shape change (the v1.24/v1.32/v1.33 precedent), all provisional. Prior 1.21 = v1.35.0 — new provisional `ai_session.usage_by_model` field (per-model token-usage attribution); a new field in an existing namespace = additive contract change (the v1.20/v1.33 precedent), provisional. Prior 1.20 = v1.34.0 — three new provisional fields in the `chatlog` namespace (first_timestamp/last_timestamp/cwd) for the recall#62 session time+project axes; new fields = additive contract change (the v1.20/v1.33 precedent), provisional. Prior 1.19 = v1.33.0 — new `ai_session` namespace (vendor/surface/models/id_prefix/object_types/schema_mismatch/usage) for AI-session observation increment 1; a new namespace = contract-shape change (the v1.24/v1.25/v1.32 precedent), all fields provisional. Prior 1.18 = v1.32.0 — new `fact_block` namespace (pair_count/pairs/duplicate_keys) for the generic kv-fact-block specialist (FR #114); a new namespace = contract-shape change (the v1.24/v1.25 precedent), new fields provisional. Prior 1.17 = v1.31.0 — promotion pass: image EXIF + the entire video namespace provisional→stable (designation-only, manifest byte-identical; the v0.11/v1.10/v1.14/v1.23 contract-change precedent). Prior 1.16 = v1.25.0 — new `audio` namespace (format/bitrate/duration_s/title/artist/album/year) for .mp3 (Candidate B ph.2); .ppt reuses the existing `presentation` fields. Prior 1.15 = v1.24.0 — new `presentation` namespace (slide_count/title/author/application) + office/image extraction routing (Candidate B ph.1). Prior 1.14 = v1.23.0 — promoted `preservation` (vector + FileRecord field) provisional→stable: a contract change (v0.11/v1.10/v1.14 precedent), designation-only so the manifest is byte-identical. Prior 1.13 = unchanged in v1.21 (recognition is LOGIC, no new field). v1.20.0 — new field video.creation_date_qt (additive). Prior 1.12 = v1.18.0 — video namespace gains make/model/gps_present/gps_source (additive); geotagged description broadens image→image+video
...
# Office MIME in BINARY_MIME_TYPES), so on the no-libmagic Windows path a plain-text
# CSV was typed binary → is_binary flipped True → the text tier (csv_headers) was
# skipped. Pin the canonical text type Linux already produces so a CSV is text on
# every OS (no-op on Linux; overrides the registry on Windows, same mechanism the
# .xls pin below proves).
mimetypes.add_type("text/csv", ".csv")
# v1.46.2: .vx (an XML-family text format fo parses for document_keys) is unknown to
# mimetypes.guess_type on EVERY platform (returns None). On Linux libmagic reads the
# content as text/plain → is_binary False; on the no-libmagic Windows path a None
# extension_mime falls back to application/octet-stream (a BINARY type) → is_binary
# flipped True (the .csv class, found by the Windows shakedown MIME sweep). Pin the
# text/plain that libmagic resolves so .vx is text on every OS — text/plain (NOT
# application/xml) keeps mime_analysis.matches_extension True against the libmagic
# reading, mirroring the .toml pin above.
mimetypes.add_type("text/plain", ".vx")
...
# v1.46: native-Windows reparse-point (junction / dir symlink) awareness for
# discovery containment. On Windows, a directory walk DESCENDS into junctions
# (unlike POSIX, where symlinked dirs are omitted) — and Path.is_symlink() returns
# False for a junction, so the v1.8.1 file-symlink containment can't see it. A
# crafted out-of-tree or cyclic junction would therefore be walked (a tree-escape,
# a double-count, or a hang/OOM). iter_files PRUNES reparse-point dirs during the
# walk (Windows only) so such a directory is never descended into in the first
# place — the containment guarantee holds before any file is materialized.
_WINDOWS = os.name == "nt"
_REPARSE_ATTR = getattr(stat, "FILE_ATTRIBUTE_REPARSE_POINT", 0x400)
# v1.46.8 (#169): the NAME-SURROGATE tag bit (IsReparseTagNameSurrogate). A reparse point
# is a "name surrogate" when it redirects the namespace to another named entity — junctions
# (IO_REPARSE_TAG_MOUNT_POINT) and directory symlinks (IO_REPARSE_TAG_SYMLINK) both set it.
# NON-surrogate reparse points (OneDrive Files-On-Demand, Data-Dedup, AppExecLink) do NOT —
# they virtualize content IN PLACE without relocating the namespace. The IO_REPARSE_TAG_*
# constants aren't in stdlib, but this bit IS the documented predicate for "redirects the
# namespace" — exactly the tree-escape / cycle class we must prune, and it future-proofs
# against any new surrogate class without hard-coding tag values (Tailnet NTFS validation).
_NAME_SURROGATE_BIT = 0x20000000

# --- v1.48.0: download-origin observation (RFC docs/v1.48.0_RFC_Specification.md) ---
#
# fo reads an ALLOWLIST of documented origin attributes. It NEVER enumerates a file's
# extended attributes (RFC §3.1): an xattr namespace is the first OPEN channel fo reads
# — any application may write any key — so enumerating would pour arbitrary, unbounded,
# un-trust-classifiable third-party metadata into a manifest consumers feed to models.
#
# Admission rule for anything added here (RFC §3.1) — ALL FOUR must hold:
#   1. documented, fixed semantics (not a free-form field)
#   2. bounded (small, known shape)
#   3. about ORIGIN or INTEGRITY (not a user's annotation)
#   4. named, never enumerated
# This admits quarantine / Zone.Identifier / xdg-origin. It excludes Finder tags,
# user.comment, SELinux labels, and app state. Criterion 1 is "documented semantics",
# deliberately NOT "written by the OS" — user.xdg.origin.url is written by Firefox/wget,
# and an OS-authorship test would exclude it while admitting nothing safer.
_MACOS = sys.platform == "darwin"

_XATTR_QUARANTINE = "com.apple.quarantine"          # macOS, documented
_XATTR_XDG_ORIGIN = "user.xdg.origin.url"           # Linux, freedesktop convention
_ADS_ZONE_IDENTIFIER = "Zone.Identifier"            # Windows, MS-documented

# A marker is tens of bytes. Anything larger is hostile or broken; cap the READ so a
# crafted 200 MB attribute can't be pulled into memory (RFC §3.7).
_ORIGIN_ATTR_MAX_BYTES = 4096

# Windows URLSecurityZone. Closed enum -> fo_derived (RFC §3.6).
_ZONE_BY_ID = {
    0: "local",
    1: "intranet",
    2: "trusted",
...
# Quarantine flag bit meaning "came from outside this machine". The low bits are a
# flags word; a nonzero value is the practical downloaded predicate.
_QUARANTINE_MAX_EPOCH = 1 << 34  # sanity bound: ~year 2514, rejects absurd timestamps


def _parse_quarantine(value: str) -> dict[str, Any] | None:
    """Parse `com.apple.quarantine` -> the origin fields, or None if unusable.

    Shape is `flags;hex-epoch;agent;event-uuid` (RFC §3.5). The event UUID is a
    machine-local identifier with no cross-machine meaning and is DROPPED.

    Never raises: a malformed marker is an observation that failed, not an error.
    """
    ...
    try:
        epoch = int(parts[1], 16)
        if 0 < epoch < _QUARANTINE_MAX_EPOCH:
            acquired_at = (
    ...
    agent = parts[2].strip() or None
    if agent is not None:
        # file_derived: bound it so a crafted agent string can't bloat the manifest.
        agent = agent[:256]

    return {
        "downloaded": bool(flags),
        "zone": "internet" if flags else None,
        "agent": agent,
        "acquired_at": acquired_at,
        "source": "macos_quarantine",
    }


def _parse_zone_identifier(text: str) -> dict[str, Any] | None:
    """Parse a Windows `Zone.Identifier` ADS body -> the origin fields, or None.

    Reads ONLY `ZoneId`. `ReferrerUrl` / `HostUrl` are deliberately NOT surfaced
    (RFC §3.3) — a URL is attacker-controllable free text arriving from outside,
    which is the payload class v1.40-v1.42 exists to contain.
    """
    if not isinstance(text, str) or "ZoneId" not in text:
        return None
    zone_id: int | None = None
    for line in text.splitlines():
        key, sep, val = line.partition("=")
        if sep and key.strip().lower() == "zoneid":
            try:
                zone_id = int(val.strip())
            except (ValueError, TypeError):
                return None
            break
    if zone_id is None:
        return None
    zone = _ZONE_BY_ID.get(zone_id)
    if zone is None:
        return None
    return {
        # ZoneId 0 is "this machine" — present marker, but NOT downloaded.
        "downloaded": zone_id >= 1,
        "zone": zone,
        "agent": None,
        "acquired_at": None,
...
def _read_zone_identifier(path: Path) -> str | None:
    """Read the Windows `Zone.Identifier` alternate data stream, bounded."""
    if not _WINDOWS:
        return None
    try:
        with open(f"{path}:{_ADS_ZONE_IDENTIFIER}", "r", encoding="utf-8", errors="replace") as fh:
            return fh.read(_ORIGIN_ATTR_MAX_BYTES)
...
def observe_origin(path: Path) -> dict[str, Any] | None:
    ...
    ALWAYS returns None rather than raising — origin is a bonus observation and must
    never be able to fail a scan (RFC §3.7).

    NOTE (RFC §4.1): extended attributes and ADS are LOST by ordinary copy/rsync/tar/
    zip. Presence is meaningful; ABSENCE IS NOT EVIDENCE the file was not downloaded.
    """
    try:
        if _MACOS:
...
def _should_prune_dir(path: Path, root_resolved: Path) -> bool:
    """Windows-only: True if the directory walk should NOT descend into ``path``.

    Prune when ``path`` is a NAME-SURROGATE reparse point — an NTFS junction or a
    directory symlink, which redirect the namespace elsewhere (the tree-escape /
    crafted-cycle / huge-target vectors). A NON-surrogate reparse point (OneDrive
    Files-On-Demand, Data-Dedup, AppExecLink, WSL) virtualizes content IN PLACE and
    must be DESCENDED, not pruned — pruning on the bare reparse BIT silently dropped
    those in-tree subtrees from the manifest with no ErrorRecord (#169, confirmed on
    native NTFS; the file-level resolve()-containment still catches any real escape).
    When attributes can't be read (inconclusive), FAIL CLOSED via a resolve()-
    containment fallback: prune if it resolves outside the tree or can't be resolved
    at all (an inaccessible dir yields nothing lost). A normal, readable in-tree
    directory returns False (descend). This closes the escape/cycle/DoS before rglob
    would have materialized a junction's subtree.
    """
    try:
        # lstat (do NOT follow) — a junction's OWN entry carries the reparse
        # attribute; os.stat would follow it to a normal target and miss it.
        st = os.lstat(path)
        if not (st.st_file_attributes & _REPARSE_ATTR):  # type: ignore[attr-defined]
            return False  # not a reparse point at all → a normal dir, descend
        # It IS a reparse point: prune ONLY name surrogates (junction / dir symlink).
        return bool(st.st_reparse_tag & _NAME_SURROGATE_BIT)  # type: ignore[attr-defined]
    except (OSError, AttributeError):
        try:
            return not path.resolve().is_relative_to(root_resolved)
        except OSError:
            return True   # unresolvable → prune (fail-closed; yields nothing)


def _write_stdout_utf8(text: str) -> None:
    """Write ``text`` to stdout as UTF-8 bytes, codepage-independent (safe on a
    legacy cp1252 Windows console, where text stdout would raise UnicodeEncodeError
    on non-ASCII metadata). Mirrors the main ``--stdout`` path. Falls back to text
    stdout when the stream has no ``.buffer`` (e.g. pytest capture). v1.46."""
...
# v1.30.2 ReDoS hardening: the inner span excludes `[`/`]`/newline so a run of unmatched
# `[[` can't force catastrophic backtracking (a 64 KB `[[`-run hung ~13 s before). A real
# wiki link `[[Page Name]]` contains no brackets and is single-line; the exclusion changes
# only bracket-bearing pathological inputs (manifest_checksum moves for those files only).
...
class FileRecord:
    ...
    # v1.48 (provisional): download-origin — {downloaded, zone, agent, acquired_at, source}.
    # ALWAYS PRESENT, None when no marker was read (RFC §3.2 / §8 D5): `null` is fo's
    # meaning-of-absence per Pillar 4, and omitting the key would be fo's first
    # conditionally-absent top-level field + a KeyError where consumers expect None.
    # Absence is NOT evidence the file wasn't downloaded — xattrs are lost by copy (§4.1).
...
SPECIALIST_FIELDS: dict[str, list[str]] = {
    ...
    "chatlog": [
        ...
        "first_timestamp", "last_timestamp", "cwd",   # v1.34 (recall#62) session axes
    ],
    ...
    "lexicon_match": ["lexicon_id", "categories", "total_hits", "total_tokens", "metadata"],  # v1.38 bring-your-own-lexicon (only when a lexicon is supplied); v1.41 adds `metadata` (file-derived self-sweep sub-block)
}
...
TRUST_MIXED = "mixed"   # `specialist_metadata` + v1.48 `origin` — classified per leaf (see the projectors)

FIELD_TRUST: dict[str, str] = {
    # v1.48: `origin` is MIXED — downloaded/zone/acquired_at/source are fo_derived
    # (booleans, closed enums, a normalized timestamp: none can carry a payload), while
    # `agent` is file_derived free text written by whatever fetched the file. The
    # scoped projection in _project_origin keeps the flag and nulls the string, so a
    # safe-mode consumer still learns the file came from outside (RFC §3.6).
    "origin": TRUST_MIXED,
...
_NESTED_TRUSTED_METADATA_KEYS: frozenset[str] = frozenset({
    ...
    # v1.44 (#146): the per-category value dict {count, density} inside lexicon_match.categories
    "count",
})
...
def _project_metadata_trusted_only(val: Any) -> Any:
    ...
    if isinstance(val, dict):
        ...
        for k, v in val.items():
            if not (isinstance(k, str) and k in TRUSTED_METADATA_KEYS):
                continue
            # v1.44 (#146): the lexicon_match namespace is consumer_config ∪ fo_derived (audited by
            # test_lexicon_match_trust_complete) — project it so the consumer's category NAMES survive.
            out[k] = _project_lexicon_match_trusted_only(v) if k == "lexicon_match" \
                else _project_metadata_trusted_only(v)
        return out
    return None


# v1.44 (#146): the fixed set of keys the lexicon_match trusted-subtree pass understands. A future
# lexicon_match field lands in neither branch below → generic projection (string→null, unknown key→drop),
# so it can NEVER leak file content; and test_lexicon_match_trust_complete fails until it's classified.
_LEXICON_MATCH_CONSUMER_CONFIG_STRINGS: frozenset[str] = frozenset({"lexicon_id"})
_LEXICON_MATCH_CATEGORY_DICTS: frozenset[str] = frozenset({"categories"})


def _project_lexicon_match_trusted_only(ns: Any) -> Any:
    """Project the `lexicon_match` namespace for --trusted-only. It carries ONLY counts (fo_derived)
    keyed by category names + a lexicon_id (consumer_config — the consumer's own runtime config, never
    bytes from a scanned file), so the whole surface is safe to keep. Relaxes the drop-unknown-keys rule
    ONLY for the `categories` sub-dict (its keys are legitimate category names) and keeps `lexicon_id`
    as the one allowed string; EVERY other leaf falls through to the generic (nulling) projector —
    safe-by-construction against any future file-derived field."""
    if not isinstance(ns, dict):
        return _project_metadata_trusted_only(ns)
    out: dict[str, Any] = {}
    for k, v in ns.items():
        if not (isinstance(k, str) and k in TRUSTED_METADATA_KEYS):
            continue
        if k in _LEXICON_MATCH_CONSUMER_CONFIG_STRINGS:
            out[k] = v if (v is None or isinstance(v, str)) else None   # consumer_config string KEPT
        elif k in _LEXICON_MATCH_CATEGORY_DICTS:
            out[k] = _project_lexicon_categories(v)                     # keep category-name keys
        elif k == "metadata":
            out[k] = _project_lexicon_match_trusted_only(v)            # same shape (categories + totals)
        else:
            out[k] = _project_metadata_trusted_only(v)                 # total_hits/total_tokens → numbers
    return out


def _project_lexicon_categories(cats: Any) -> Any:
    """{category_name: {count, density}} — KEEP the consumer-config category-name keys, project the inner
    {count, density} generically (the numbers survive; any string leaf still nulls)."""
    if not isinstance(cats, dict):
        return _project_metadata_trusted_only(cats)
    return {k: _project_metadata_trusted_only(v) for k, v in cats.items() if isinstance(k, str)}


_ORIGIN_TRUSTED_LEAVES = ("downloaded", "zone", "acquired_at", "source")


def _project_origin_trusted_only(o: Any) -> Any:
    """v1.48: scoped projection for `origin` (TRUST_MIXED) — the v1.44 lexicon pattern.

    `agent` is the lone file_derived leaf: free text written by whatever fetched the
    file, so an attacker who controls the download influences it. Everything else is
    fo_derived — a boolean, two closed enums, and a normalized timestamp, none of which
    can carry a payload.

    Without this scoping the generic leaf-nuller would also null `zone`/`source`/
    `acquired_at` (they are strings), destroying the routing signal safe mode exists to
    deliver: a consumer must still learn THAT the file came from the internet without
    ingesting the agent string (RFC §3.6).

    Any UNKNOWN leaf nulls — safe by construction if the block ever grows a field.
    """
    if not isinstance(o, dict):
        return None
    return {k: (o.get(k) if k in _ORIGIN_TRUSTED_LEAVES else None) for k in o}
...
def _project_file_record_trusted_only(fr: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, val in fr.items():
        ...
        if key == "origin":
            # v1.48: scoped — keeps the fo_derived routing signal, nulls only `agent`.
            out[key] = _project_origin_trusted_only(val)
        elif trust == TRUST_MIXED:
...
def _scrub_manifest_level_trusted_only(d: dict[str, Any]) -> dict[str, Any]:
    ...
    for vector in d.get("vectors_collected") or []:
        # v1.44 (#146): the `lexicon` vector summary is consumer_config ∪ fo_derived (lexicon_id +
        # category names + counts; NO bytes from a scanned file) → KEEP it so a safe-mode consumer can
        # route by category. Every OTHER corpus rollup is laced with file-derived author/producer names.
        if isinstance(vector, dict) and vector.get("vector_id") != "lexicon":
            vector["summary"] = {}
    return d
...
PROVISIONAL_SPECIALIST_FIELDS: frozenset[tuple[str, str]] = frozenset({
    ...
    ("chatlog", "alternation"),
    ("chatlog", "first_timestamp"),   # v1.34 (recall#62) session axes — provisional
    ("chatlog", "last_timestamp"),
    ("chatlog", "cwd"),
    # v1.31.0 promotion pass: the v1.16 image-EXIF fields (make/model/orientation/
    # datetime_original/gps_present/xmp_present) AND the entire v1.17–1.20 `video` namespace
    # (codec/duration_s/width/height/creation_date/creation_date_qt/make/model/gps_present/
    # gps_source) were PROMOTED provisional→stable — settled logic (unchanged since ship) +
    # exiftool-oracle-validated + corpus-proven (wikimedia-exif + synth-video-capture) +
    # red-teamed (test_capture_metadata_hardening). No longer listed here → `--schema`
    # annotates them `stable`. (Image dimensions width/height/bit_depth were already stable
    # since 0.5.) Designation-only; manifest byte-identical (stability lives only in `--schema`).
    # v1.47.0 promotion pass: the v1.24 `presentation` namespace (slide_count/title/author/
    # application) AND the v1.25 `audio` namespace (format/bitrate/duration_s/title/artist/album/
    # year) were PROMOTED provisional→stable — settled logic (extraction unchanged since ship;
    # v1.25.1's OLE2 patch changed provenance ACCURACY, not any extracted value), corpus-swept every
    # release, and hardened (presentation reuses the docx/xlsx ZIP+XML + OLE2 machinery; audio was the
    # lone net-new binary parser carried at the v1.8.1 bounded bar + folded into
    # test_capture_metadata_hardening). No longer listed here → `--schema` annotates them `stable`.
    # These are exactly the fields v1.31.0 §6 deferred as "presentation/audio — too young, season next
    # pass"; this is that pass. Designation-only; manifest byte-identical (stability lives only in `--schema`).
    ...
    ("lexicon_match", "metadata"),   # v1.41: file-derived metadata self-sweep — provisional on arrival
})
...
PROVISIONAL_MANIFEST_FIELDS: frozenset[tuple[str, str]] = frozenset({
    ...
    # signature vocabulary intentionally grows every MIME change, so freezing it would be wrong.
    ("FileRecord", "origin"),              # v1.48 — new; provisional until the shape settles
    ("FileRecord", "format_signatures"),  # §2.4 — internal magic scan results (vocabulary stays fluid)
    ("FileRecord", "is_polyglot"),         # §2.4 — derived from format_signatures
})
...
class _Within:
    """v1.23.1 / v1.23.2 — a bounded, optionally-corroborated find-anywhere: the pattern must
    START within the first `n` bytes AND (when `corroborate` is set) the sample must ALSO
    contain at least one corroborating token. ENFORCED in the MIME sniff (C1, `_sniff_mime`);
    IGNORED in `scan_signatures` (C2, `enforce_window=False`) — a real signature embedded deep
    in a polyglot must still register its `format_signature` so `is_polyglot` stays honest. So
    the MIME judgment is sharpened WITHOUT suppressing the polyglot OBSERVATION (observe-don't-
    interpret).

    For `%PDF-`: v1.23.1 bounded it to a 256-byte window (a stray deep `%PDF-` literal in a
    source/text file must NOT type the file `application/pdf` — the FP puresniff's clean-room
    sweep found, loose since v1.3). v1.23.2 widens the window to 1024 — matching the scanner's
    OWN PDF-header tolerance (`_extract_pdf_metadata` searches `sample[:1024]`) — AND adds
    structural corroboration, so a real PDF with a junk/BOM prefix up to 1024 bytes is now
    correctly typed while the deep literal (no PDF structure) is still rejected. Offset alone
    couldn't separate the two (the corpus FP literal sat at 864, inside 1024); offset +
    corroboration can. Measured: max real-PDF `%PDF-` offset 128 across 1012 corpus PDFs, so
    1024 keeps every real PDF with ample headroom."""
    __slots__ = ("n", "corroborate")
    def __init__(self, n: int, corroborate: tuple[bytes, ...] = ()) -> None:
        self.n = n
...
# rejected even though it now falls inside the widened 1024-byte window.
...
# v1.46.4: the leading magics of macOS-universal metadata (.DS_Store = Bud1; ._* = AppleDouble).
# libmagic types both as application/octet-stream, so the v1.22 recognition gate (which keys off the
# content MIME) would flag them unsupported_extension on every Mac corpus scan. A magic match IS a
# positive content ID (per the gate's own comment), so these count as identified even when libmagic
# shrugs. Mirrors the two MAGIC_SIGNATURES entries below (kept in sync — both are fixed format magics).
_MACOS_METADATA_MAGICS: tuple[bytes, ...] = (b"\x00\x00\x00\x01Bud1", b"\x00\x05\x16\x07")
...
MAGIC_SIGNATURES: list[tuple[tuple[tuple[int | None | _Within, bytes | _OneOf], ...], str]] = [
    ...
    # macOS Finder / resource metadata (v1.46.4): macOS scatters these across nearly
    # every directory, so recognizing them by magic stops the unsupported_extension
    # error-record spray on Mac corpora (surfaced by the MacBook Neo APFS shakedown).
    # .DS_Store = Bud1 (Finder folder state); ._* = AppleDouble (RFC 1740 resource fork
    # / xattr sidecar on non-native filesystems). Both start with a NUL → no prose collision.
    (((0, b"\x00\x00\x00\x01Bud1"),), "application/x-apple-dsstore"),
    (((0, b"\x00\x05\x16\x07"),), "application/applefile"),
...
# v1.46.7 — MIME synonym classes: labels that denote the SAME on-disk format under
# different IANA/vendor spellings. `analyze_mime`'s content-vs-extension check was a
# strict `detected == extension_mime`, so a legitimate synonym was reported as a
# content-vs-extension MISMATCH → the file scored "degraded". Surfaced by a real jsonl
# chatlog corpus (recall's LongMemEval synthesis) scoring 0% clean — 19,195/19,195 false
# mismatches, ALL JSON-family synonyms: libmagic types `.jsonl` as application/x-ndjson
# (multi-line) or application/json (single-line) while the extension map registers
# application/jsonl. A mismatch must mean a GENUINE format disagreement (a `.pdf` that is
# really a zip), not two labels for compatible content. Five classes are strict SAME-format-
# different-label pairs (xml, rtf, markdown, python-source, python-bytecode). The JSON class
# is deliberately BROADER than a spelling synonym: application/json (single doc) vs
# application/x-ndjson (newline-delimited multi-doc) are different serializations, BUT
# libmagic's split between them is an UNRELIABLE line-count heuristic (a pretty-printed single
# JSON doc types as x-ndjson; a minified stream as json), all three are JSON-family text, and
# fo's chatlog/ai_session guards already consume them interchangeably (SPECIALIST_MIME_GUARDS)
# — so treating a `.json` whose content libmagic calls x-ndjson (or vice-versa) as a content-
# vs-extension DISAGREEMENT would be the same false signal as the .jsonl case. This is a JSON-
# FAMILY fold, not a pure synonym (leg-1 v1.46.7, accepted tradeoff). NOT folded: supertype
# relationships (text/plain for a text-based format like markdown/yaml, which libmagic can't
# sub-type) — a separate, broader question deliberately left untouched here.
MIME_SYNONYM_CLASSES: tuple[frozenset[str], ...] = (
    frozenset({"application/json", "application/jsonl", "application/x-ndjson"}),
    frozenset({"text/xml", "application/xml"}),
    frozenset({"text/rtf", "application/rtf"}),
    frozenset({"text/markdown", "text/x-markdown"}),
    frozenset({"text/x-python", "text/x-script.python"}),
    frozenset({"application/x-python-code", "application/x-bytecode.python"}),
)
_MIME_SYNONYM_INDEX: dict[str, frozenset[str]] = {
    mime: cls for cls in MIME_SYNONYM_CLASSES for mime in cls
}


def _mime_labels_compatible(detected: str, expected: str) -> bool:
    """True when a content-detected MIME and an extension-derived MIME describe the SAME
    format — identical, or two labels within one ``MIME_SYNONYM_CLASSES`` set — i.e. NOT a
    genuine content-vs-extension disagreement. Used by ``analyze_mime`` so
    ``matches_extension`` flags only real mismatches, not synonym spellings (v1.46.7).
    A binary content type for a text extension (or any cross-format pair) is NOT in a
    synonym class → still flags, so real mismatches (a `.txt` that is really a PNG) survive."""
    if detected == expected:
        return True
    cls = _MIME_SYNONYM_INDEX.get(detected)
    return cls is not None and expected in cls
...
AI_SESSION_METHOD_VERSION = 2   # v1.35: per-model usage attribution (usage_by_model) added to the block's rules
...
LEXICON_METHOD_VERSION = 2   # v1.41: metadata self-sweep is a new rule in the mechanism (1→2)
LEXICON_SAFETY_FLAG = "lexicon_match"   # neutral presence bit (a fact: matched the supplied lexicon body OR metadata)
...
LEXICON_METADATA_MAX_CHARS = 65536
...
def lexicon_rules_fingerprint() -> str:
    ...
    return "|".join([
        ...
        "flag=lexicon_match@body_or_metadata_total_hits>=1",   # v1.41: fires on a body OR metadata hit
        "read=full_file_bounded_deviation",
        # v1.41: the metadata self-sweep — reuse the matcher over the file-derived metadata strings
        # (filename+tags+frontmatter+structural+specialist_metadata string leaves, minus lexicon_match
        # and content_preview), density over metadata tokens.
        "metadata_sweep=filename+tags+frontmatter+structural+specialist_metadata_string_leaves-content_preview",
        f"metadata_max_chars={LEXICON_METADATA_MAX_CHARS}",
...
def _read_lexicon_index(idx: Path) -> tuple[list[Path], str | None]:
    ...
    if text.lstrip()[:1] == "{":
        ...
        for lineno, raw_line in enumerate(text.splitlines(), 1):
            ...
            if not line or line[0] in "!#":
                continue
            rels.append(line)
    # v1.46.6 (bestiary soak ③): confine index members to the index's OWN directory subtree — an
    # index is a self-contained subscription BUNDLE (the v1.43 model), and the RFC deferred `!#include`
    # to avoid a "traversal surface" but the `sources` list itself could `../`-escape (or use an
    ...
    for r in rels:
        ...
        if not m.resolve().is_relative_to(base_resolved):
            raise ValueError(
                f"lexicon index member {r!r} escapes the index directory {base} — index members must "
                f"stay within the index's own directory (a self-contained subscription bundle); use a "
                f"separate --lexicon flag for an out-of-bundle source"
            )
        members.append(m)
    return members, lid
...
# v1.34.0: session axes (recall#62). Recognized per-turn timestamp keys (measure-first grounded:
# Claude Code `timestamp`, oasst `created_date`, ChatGPT export `create_time`, + common `created_at`;
# generic `date`/`time`/`created` EXCLUDED = FP risk). cwd = the session's working directory.
CHATLOG_TIMESTAMP_KEYS: frozenset[str] = frozenset({"timestamp", "created_date", "create_time", "created_at"})
CHATLOG_SESSION_AXIS_FIELDS: frozenset[str] = frozenset({"first_timestamp", "last_timestamp", "cwd"})
# v1.34 review (leg-1 + leg-2 CONVERGED): do NOT collect timestamps/cwd from inside tool payloads —
# a `tool_result` carrying a `gh api` JSON object (created_at) or a `tool_use` input carrying a cwd
# would pollute the session span/dir. Skip descending into these content blocks. (Measure-first: 0 real
# federation logs embed nested timestamp keys today, but two decorrelated legs flagged it → defensive.)
CHATLOG_TIMESTAMP_SKIP_BLOCKS: frozenset[str] = frozenset({"tool_use", "tool_result"})
# The temporal read is HEAD (reused full_text) + a TAIL slice for >64 MiB files — chatlog turns are
# chronological, so `last_timestamp` lives at the END; a front-only read truncated it (leg-1 + real-data
# verify: a 141 MB / 2-day session reported a partial span). Bounded (the v1.5 PDF head+tail precedent).
CHATLOG_TAIL_BYTES = 8 * 1024 * 1024   # tail slice read for a >64 MiB session
CHATLOG_CWD_MAX_STR = 4096          # cap the surfaced cwd (PATH_MAX-class; bounded observation)
CHATLOG_AXES_MAX_NODES = 2_000_000  # bounded walk (the ai_session cap) over a full 64 MiB session


def _parse_timestamp_utc(value: Any) -> Any:
    """Parse a recognized turn-timestamp value → aware UTC datetime. Accepts an ISO-8601 string
    (incl. a trailing `Z`, tz offsets, ms/µs precision) OR a numeric epoch (int/float seconds).
    Returns None on anything else — never crashes (the v1.8.1 bounded/never-crash bar)."""
    from datetime import datetime, timezone
    if isinstance(value, bool):        # bool is an int subclass — reject
        return None
    if isinstance(value, (int, float)):
        try:
            if not (0 <= float(value) < 1e13):   # reject absurd/hostile epoch magnitudes
                return None
    ...
    if isinstance(value, str):
        s = value.strip()
        if s[-1:] in ("Z", "z"):       # 3.10 fromisoformat doesn't accept Z; normalize
            s = s[:-1] + "+00:00"
        try:
            dt = datetime.fromisoformat(s)
        except (ValueError, TypeError):
            return None
        if dt.tzinfo is None:          # naive → assume UTC (fo's capture-metadata stance)
            dt = dt.replace(tzinfo=timezone.utc)
        return dt.astimezone(timezone.utc)
    return None


def _canonical_iso_ms(dt: Any) -> str:
...
CHATLOG_RULES_DEFINITION = (
    ...
    # v1.34.0: session axes — min/max turn timestamp over the recognized-key set → canonical
    # ISO-8601 UTC; first-seen cwd. Key set + normalization + cwd cap feed the rules_hash.
    "session_axes{first_timestamp,last_timestamp,cwd(first_seen)};"
    "timestamp_keys:created_at,created_date,create_time,timestamp;"
    "timestamp_norm:parse(iso|epoch)->utc->canonical_iso8601_ms_Z,min/max,null_if_none;"
    "cwd_max:" + str(CHATLOG_CWD_MAX_STR) + ";"
...
# v1.30.2 ReDoS hardening: the outer `\s+` and the inner `\s*` overlapped, so a long
# whitespace run (a crafted PDF /Producer of 64 KB of tabs/spaces) backtracked
# catastrophically. Bounding both whitespace runs ({1,32}/{0,16}) makes it linear;
# verified byte-identical to the old pattern on real producer strings AND the
# leading-whitespace edge (a version preceded by > 32 whitespace chars is not real).
PROVENANCE_VERSION_SUFFIX_RE = re.compile(r"\s{1,32}(v(er)?\.?\s{0,16})?\d.*$", re.I | re.S)
...
def run_watch(source_dir: Path, config: "ScannerConfig") -> int:
    ...
    def _emit(manifest_dict: dict, force_include_files: bool = False) -> None:
        ...
        # v1.46: byte-safe (UTF-8) so a non-ASCII delta doesn't crash on a cp1252 console.
        _write_stdout_utf8(_json.dumps(out, separators=(",", ":")) + "\n")
        ...
        for tag, name in _EXIF_IFD0_TAGS.items():
            if tag in ifd0:
                ...
                if name == "orientation":
                    ...
                    # malformed → None (NOT an int — bestiary #165: an int here crashed the summary's
                    # .strip() at scan-abort scope; the field must stay str|None, never int).
                    out[name] = _exif_ascii(raw) if typ == 2 else None
...
class Scanner:
    ...
    def _build_summary(self, manifest: 'ScanManifest') -> str:
        ...
        for v in manifest.vectors_collected:
            ...
            elif vid == "lexicon":   # v1.45 (#147): the lexicon vector's shape (the flag count already shows)
                matched = summary.get("files_matched", 0)
                cat_hits = summary.get("category_hits", {}) or {}
                if matched:
                    n_cats = sum(1 for c in cat_hits.values() if c)
                    total_hits = sum(cat_hits.values())
                    vec_parts.append(f"lexicon matched {matched} files "
                                     f"({n_cats} categor{'y' if n_cats == 1 else 'ies'}, {total_hits} hits)")
        if vec_parts:
        ...
        # make/model are str|None by construction, but this is the AGGREGATE path — a single bad file
        # must never abort the whole scan (bestiary #165: a numeric-typed EXIF make → int → .strip()
        # crash at scan-abort scope). Coerce defensively: only string values contribute to a device
        # label; anything else is ignored. Empty labels are dropped.
        def _mm(v: Any) -> str:
        ...
        devices = sorted({
            ...
            for d in [f"{_mm(md.get('make'))} {_mm(md.get('model'))}".strip()]
            if d
        })
        cap_parts: list[str] = []
        if geo:
        ...
        # v1.45 (#147): fact-block files — generic kv-pair bodies (v1.32).
        fact_blocks = sum(1 for r in manifest.files if getattr(r, "is_fact_block", False))
        if fact_blocks:
            lines.append(f"Fact blocks: {fact_blocks} file{'s' if fact_blocks != 1 else ''}.")

        # v1.45 (#147): AI-session logs — token-usage SUMS. Counted, NEVER priced (the charter).
        _ai_component_keys = ("input_tokens", "output_tokens", "cache_read_tokens",
                              "cache_creation_tokens", "reasoning_tokens")
        ai_sessions = [ais for r in manifest.files if r.specialist_metadata
                       for ais in (r.specialist_metadata.get("ai_session"),)
                       if isinstance(ais, dict)]   # leg-4/gemini: an ai_session value may be None
        if ai_sessions:
            tokens = 0
            for ais in ai_sessions:
                usage = ais.get("usage") or {}
                # leg-4/Codex: prefer a vendor total (OpenAI `total_tokens`); else sum the components (Anthropic)
                vendor_total = usage.get("total_tokens")
                tokens += vendor_total if vendor_total is not None else sum(usage.get(k) or 0 for k in _ai_component_keys)
            lines.append(f"AI sessions: {len(ai_sessions)} log{'s' if len(ai_sessions) != 1 else ''}, "
                         f"{tokens:,} tokens observed (never priced).")
    ...
    def _build_context(self) -> ScanContext:
        ...
        # purexml (v1.36: replaced defusedxml as the XML-hardening backend). Its presence + version
        # AND the applied structural caps GATE the XML parse (a capped depth/size can degrade a field
        # on pathological input), so both are recorded — the ScanContext explains that variance
        # (Pillar 1) + the caps are auditable, not implicit ([[project_rules_fingerprint_lens]]).
        if _xml_backend == "purexml":
            try:
                import purexml
                px_ver = getattr(purexml, "__version__", "unknown")
            except Exception:
                px_ver = "unknown"
            lim = XML_STRUCTURAL_LIMITS
            deps["purexml"] = {"available": True, "version": _dep_version_str(px_ver),
                               "limits": {"max_depth": getattr(lim, "max_depth", None),
                                          "max_attributes": getattr(lim, "max_attributes", None),
                                          "max_bytes": getattr(lim, "max_bytes", None)}}
        else:
            # no purexml → the unhardened stdlib fallback (no structural caps)
            deps["purexml"] = {"available": False, "version": None, "limits": None}
    ...
    def iter_files(self, root: Path) -> Iterable[Path]:
        ...
        if self.config.skip_output_dir is not None:
            try:
                ...
                skip_rel_parts = None
        # v1.46: enumerate via os.walk (topdown) instead of rglob so reparse-point
        # DIRECTORIES (NTFS junctions / dir symlinks) can be PRUNED before descent.
        # rglob would fully walk + materialize a junction's target first, so a
        # crafted out-of-tree or cyclic junction escaped / double-counted / hung the
        # scan before any per-file filter ran (Codex P1). os.walk(followlinks=False)
        # preserves POSIX behavior exactly (symlinked dirs omitted, as rglob did);
        # the file-symlink containment below is unchanged. onerror is swallowed
        # (unreadable subdir skipped, never raised — matching rglob; never-crash).
        candidates: list[Path] = []
        for dirpath, dirnames, filenames in os.walk(root, followlinks=False, onerror=lambda _e: None):
            base = Path(dirpath)
            if _WINDOWS and dirnames:
                # Prune reparse-point subdirs so a junction is never descended into:
                # closes the tree-escape, the in-tree double-count, AND the crafted-
                # cycle / huge-target DoS — all BEFORE materializing the subtree.
                # Fail-closed via resolve()-containment on an inconclusive attribute
                # read (CodeRabbit). POSIX never reaches this (os.walk already omits
                # symlinked dirs); the check is one lstat per DIRECTORY, not per file.
        ...
        # case-INSENSITIVELY (via _str_normcase), so a raw sort gives a different
        ...
        # every OS (and byte-identical to the prior rglob order on POSIX).
        for path in sorted(candidates, key=lambda p: p.as_posix()):
            # os.walk 'filenames' are non-directory entries — may include a FIFO/
            # socket/broken symlink; is_file() (follows symlinks) keeps only regular
            # files, exactly as the prior `rglob("*") + is_file()` guard did.
            if not path.is_file():
                continue
            # is_file() FOLLOWS symlinks. A file symlink whose target resolves
            # OUTSIDE the scan tree would read that file's bytes/hash into the
            # manifest (a tree-escape — e.g. /etc/passwd) and can break determinism
            # if the target mutates. Enforce resolve()-containment, mirroring
            # _is_safe_zip_entry for ZIP paths (red-team #6, v1.8.1). (Reparse-point
            # DIRS are already pruned above, so this covers the file-symlink case.)
            if path.is_symlink():
                try:
    ...
    def scan_file(self, path: Path) -> FileRecord:
        ...
        content_identified = (
            ...
            # v1.46.4: fo's OWN magic positively identifies macOS-universal metadata
            # (.DS_Store / AppleDouble) that libmagic types octet-stream — a signature
            # match IS a positive ID, so it doesn't spray unsupported_extension on Macs.
            not read_failed and sample.startswith(_MACOS_METADATA_MAGICS)
        )
        ...
        if not is_binary:
            try:
                    ...
                    if is_chatlog:
                        ...
                        if self.config.enable_specialists:
                            ...
                            if mime_type not in chatlog_guard:
                                ...
                                # v1.34: read the FULL file once (bounded deviation, 64 MiB cap). Session
                                # logs far exceed the 64 KB baseline window, so the timestamp min/max —
                                # and the ai_session usage sums — MUST see every turn, not a partial
                                # window (a windowed max gives the wrong session span). The windowed
                                # `text` still drives the EXISTING chatlog signals (turn_count, speaker
                                # stats — unchanged, byte-identical); only the NEW session axes use full_text.
                                try:
                                    _, full_text, _ = self.decode_text(sample, path, AI_SESSION_MAX_FILE_BYTES)
                                    full_over_cap = path.stat().st_size > AI_SESSION_MAX_FILE_BYTES
                                except (OSError, ValueError):
                                    full_text, full_over_cap = text, True   # fell back to the window
                                # v1.34: for the timestamp axes, append a TAIL slice when the file exceeds
                                # the 64 MiB read — chatlog turns are chronological so `last_timestamp` is
                                # at the END, beyond a front-only read. head (full_text) + tail captures both
                                # extremes, bounded (the v1.5 PDF head+tail precedent). axes_partial = the
                                # tail couldn't be read (last_timestamp may be understated → honest provenance).
                                temporal_text = full_text
                                axes_partial = False
                                if full_over_cap:
                                    try:
                                        with open(path, "rb") as _tf:
                                            _tf.seek(-CHATLOG_TAIL_BYTES, 2)
                                            _tail = _tf.read(CHATLOG_TAIL_BYTES)
                                        temporal_text = full_text + "\n" + _tail.decode("utf-8", "replace")
                                    except OSError:
                                        axes_partial = True   # tail unreadable → last_timestamp partial
                                chatlog_meta = self._extract_chatlog_metadata(text, temporal_text=temporal_text)
                                if chatlog_meta is None:
                                    ...
                                    for key in chatlog_meta:
                                        # v1.34: the session-axis fields come from the FULL-file read
                                        # (a declared deviation), the rest from the bounded window.
                                        if key in CHATLOG_SESSION_AXIS_FIELDS:
                                            _p = ProvenanceEntry(layer="derived", method="_extract_chatlog_temporal",
                                                trigger="bounded_deviation",
                                                detail={"tool": CHATLOG_TOOL, "reason": "chatlog_head_tail_read",
                                                        "max_bytes": AI_SESSION_MAX_FILE_BYTES,
                                                        "tail_bytes": CHATLOG_TAIL_BYTES, "truncated": axes_partial})
                                        else:
                                            _p = ProvenanceEntry(layer="derived", method="_extract_chatlog_metadata",
                                                trigger="bounded_text", detail={"tool": CHATLOG_TOOL, "text_chars": text_len})
                                        provenance[f"specialist_metadata.{CHATLOG_NAMESPACE}.{key}"] = asdict(_p)
                                    ...
                                    # v1.33/v1.34: reuse the full-file read above (single read, no double I/O).
                                    ai_session = self._extract_ai_session(full_text, truncated=full_over_cap)
                ...
                if extension in {".md", ".mdx"}:
                    ...
                    provenance["structural.csv_headers"] = asdict(ProvenanceEntry(
                        layer="derived", method="extract_csv_headers",
                        trigger="csv_header_row",
                    ))
        ...
        if self.config.enable_specialists:
            ...
            try:
                ...
                if raw_metadata is not None:
                    ...
                    elif extension in {".doc", ".xls", ".msg", ".ppt"}:
                        # OLE2 specialists hand `olefile` the path and read property streams from
                        # anywhere in the compound file (the FAT chains span it) — NOT the 8 KB
                        # sample. Declared a deviation so the per-field provenance is honest (it
                        # previously reported `bounded_sample`/`sample_size`, false for real files).
                        # No fixed byte cap — bounded by the file size on disk → budget is None.
                        is_deviation = True
                        dev_reason, dev_budget = "ole2_full_file_required", None
                    else:
                        is_deviation = False
        ...
        # v1.41: lexicon self-sweep over the file-derived METADATA. A risk term can hide where the
        # body scan (v1.38) structurally never looks — a filename, an EXIF make/model, a PDF
        # producer/title/author, an office application string. Reuse the pure `_lexicon_scan` matcher
        # over the collected metadata strings and report the result in a `metadata` sub-block of the
        # lexicon_match namespace, kept SEPARATE from the body so a consumer sees WHERE. Runs whenever
        # a lexicon is supplied — INCLUDING on binary files that get no body scan (they still carry
        # metadata hits). When the body was never scanned the namespace doesn't exist yet, so create
        # it with a ZERO body block (`_lexicon_scan("")`) → the body+metadata shape stays consistent.
        # Bounded: the joined metadata text is capped at LEXICON_METADATA_MAX_CHARS (a hostile PDF
        # could carry a huge producer). Dormant without a lexicon → existing manifests byte-identical.
        metadata_total_hits = 0
        if self._lexicon is not None:
            md_text = self._collect_metadata_strings(
                path.name, tags, frontmatter, structural, specialist_metadata
            )
            if len(md_text) > LEXICON_METADATA_MAX_CHARS:
                md_text = md_text[:LEXICON_METADATA_MAX_CHARS]
            md_result = self._lexicon_scan(md_text)
            metadata_total_hits = md_result["total_hits"]
            if specialist_metadata is None:
                specialist_metadata = {}
            lex_ns = specialist_metadata.get(LEXICON_NAMESPACE)
            if lex_ns is None:
                # binary / never-body-scanned file: create the namespace with a zero body block so
                # the shape (body + metadata) is identical to a text file's. `_lexicon_scan("")`
                # yields the canonical zero result (all categories count 0, total_hits/tokens 0).
                zero_body = self._lexicon_scan("")
                lex_ns = {
                    "lexicon_id": self._lexicon["lexicon_id"],
                    "categories": zero_body["categories"],
                    "total_hits": zero_body["total_hits"],
            ...
            lex_ns["metadata"] = {
                ...
                "total_tokens": md_result["total_tokens"],
            }
            provenance[f"specialist_metadata.{LEXICON_NAMESPACE}.metadata"] = asdict(ProvenanceEntry(
                layer="derived",
                method="_lexicon_scan",
                trigger="lexicon_metadata_sweep",
                detail={"tool": LEXICON_TOOL,
                        "lexicon_id": self._lexicon["lexicon_id"],
                        "dictionary_id": self._lexicon_dictionary_id,
                        "max_chars": LEXICON_METADATA_MAX_CHARS,
                        "fields": ["filename", "tags", "frontmatter", "structural", "specialist_metadata"]},
            ))
        ...
        # deterministic safety_flags order (some paths above don't re-sort). v1.41: fires on a body
        # OR metadata hit (a hit is a hit — the file tripped the lexicon; presence, not verdict).
        if (lexicon_total_hits >= 1 or metadata_total_hits >= 1) and LEXICON_SAFETY_FLAG not in safety_flags:
    ...
    def analyze_mime(self, path: Path, detected_mime: str, extension: str) -> MimeAnalysisRecord:
        ...
        if extension_mime is None:
            ...
            # v1.46.7: not a strict `==` — two labels for the SAME format (e.g. the JSON
            # family application/jsonl ≡ application/x-ndjson ≡ application/json) are a match,
            # so a mismatch means a genuine content-vs-extension disagreement, not a synonym.
            matches = _mime_labels_compatible(detected_mime, extension_mime)
    ...
    def _prose_dialogue(cls, text: str) -> bool:
        ...
        # v1.49 CONSIDERED AND REVERTED (RFC §3.2, FALSIFIED): accepting ALTERNATION as
        # an alternative to recurrence recovers the all-distinct roll-call FN — and
        # reopens the i18n/multi-language FP that v1.2.2 closed. `English: The quick
        # brown fox.` / `Portuguese: A rápida raposa.` / `Spanish: El zorro rápido.`
        # alternates, and every value IS utterance-shaped (sentence punctuation), so
        # every downstream gate passes it. The recurrence floor was the only thing
        # rejecting it. That trades a REAL-CORPUS FP (tika multi-language.txt) for a
        # SYNTHETIC FN — the wrong direction. Recurrence stays REQUIRED.
    ...
    def _detect_chatlog_pattern(self, text: str) -> bool:
        ...
        # v1.49 CONSIDERED AND REVERTED: gating this co-signal on `_is_utterance` of
        # each label's value (RFC §3.1) rejects the tech-doc FP (`Platform: Linux`) but
        # ALSO rejects real terse transcripts (`Alice: hi` / `Bob: hello`) — measured:
        # `hi`, `hello`, `Linux`, `Draft`, `cold` are ALL non-utterances. The gate's
        # power comes from function words, punctuation and length, and terse dialogue
        # has none of them either. v1.4.0 already recorded this class as "irreducibly
        # ambiguous with atomic data"; the utterance gate cannot separate it, and the
        # v1.2.1 regression guard caught the attempt. See RFC §3.1 (FALSIFIED).
    ...
    def _detect_conversational_json(self, text: str) -> bool:
        ...
        if (len(speakers) >= 3 and len(set(speakers)) >= 2
            ...
            return True
        # v1.49 (§3.3): the >=3 floor COUNTS objects instead of reading them, which
        # false-negatives short real sessions — quantified on LongMemEval: 0/7 at two
        # turns, 100% at >=3. The floor is not lowered, it is made CONDITIONAL on
        # evidence that is stronger in KIND: two turns qualify only when the roles are
        # recognised conversational values AND every turn's content is utterance-shaped.
    ...
    def _extract_ai_session(self, text: str, truncated: bool = False) -> dict[str, Any] | None:
        ...
        usage_models: list[str | None] = []   # v1.35: the model co-located with each usage dict (parallel to usage_dicts)
        seen = [0]
                    ...
                    for uk in ("usage", "usageMetadata"):
                        ...
                        if isinstance(u, dict) and any(k in AI_SESSION_USAGE_KEYS for k in u.keys()):
                            if len(usage_dicts) < AI_SESSION_MAX_USAGE_DICTS:
                                ...
                                # v1.35: capture the model CO-LOCATED on this same object (Claude Code
                                # `message.model` / OpenAI `response.model` / Gemini `.modelVersion` — all
        ...
        # --- per-model usage attribution (v1.35, increment 3) ---
        # Group the co-located model with each usage dict; sum per group with the SAME
        # token_at / AI_SESSION_USAGE_MAP as the session sums above (the tested v1.33 session path
        # is left untouched, so `usage` stays byte-identical). INVARIANT: `usage` == elementwise sum
        # of `usage_by_model` (falsify-first tested). Model VERBATIM, incl. markers like `<synthetic>`
        # (observe-don't-interpret — fo emits the model field as-is; the interpreter decides "real");
        # a usage-turn naming no model → the `model: null` bucket. Cardinality backstop: beyond
        # AI_SESSION_MAX_MODELS distinct models an overflow folds into null (sums stay complete →
        # invariant holds), bounding output on hostile per-turn-unique-model input (v1.8.1 discipline).
        usage_by_model: list[dict[str, Any]] = []
        if usage_dicts:
            groups: dict[Any, dict[str, Any]] = {}
            real_model_count = 0
            for m, u in zip(usage_models, usage_dicts):
                key = m
                if key is not None and key not in groups:
                    if real_model_count >= AI_SESSION_MAX_MODELS:
                        key = None   # cardinality backstop → unattributed bucket
                    else:
                        real_model_count += 1
                b = groups.get(key)
                if b is None:
                    b = groups[key] = {"turns": 0, "sums": {}, "present": set(), "raw_keys": set()}
                b["turns"] += 1
                b["raw_keys"].update(k for k in u.keys() if isinstance(k, str))
                for kind, paths in AI_SESSION_USAGE_MAP.items():
                    for path in paths:
                        v = token_at(u, path)
                        if v is not None:
                            b["sums"][kind] = b["sums"].get(kind, 0) + v
                            b["present"].add(kind)
                            break
            # deterministic order: real model strings sorted, the null bucket LAST
            for m in sorted(groups, key=lambda x: (x is None, x or "")):
                b = groups[m]
        ...
        return {
            ...
            "usage": usage,
            "usage_by_model": usage_by_model,   # v1.35: per-model attribution (invariant: == sum == usage)
        }

    def _extract_chatlog_temporal(self, text: str) -> dict[str, Any]:
        """v1.34 — chatlog session axes: min/max turn timestamp (recognized-key set, normalized to
        canonical ISO-8601 UTC) + first-seen `cwd`, over the log's JSON objects. Deterministic pure
        function of the file; bounded/never-crash (capped walk, non-parseable values skipped). Non-JSON
        (prose) → all None. Returns {first_timestamp, last_timestamp, cwd}, each None if absent."""
        nulls = {"first_timestamp": None, "last_timestamp": None, "cwd": None}
        if not text:
            return dict(nulls)

        state = {"min": None, "max": None, "cwd": None}
        seen = [0]
            ...
            while stack and seen[0] < CHATLOG_AXES_MAX_NODES:
                ...
                if isinstance(cur, dict):
                    ...
                    for k in CHATLOG_TIMESTAMP_KEYS:
                        if k in cur:
                            dt = _parse_timestamp_utc(cur[k])
                            if dt is not None:
                                if state["min"] is None or dt < state["min"]:
                                    state["min"] = dt
                                if state["max"] is None or dt > state["max"]:
                                    state["max"] = dt
                    if state["cwd"] is None:
                        cv = cur.get("cwd")
                        if isinstance(cv, str):
                            state["cwd"] = cv[:CHATLOG_CWD_MAX_STR]
                    # reversed → DFS visits siblings/lines in file order (so first-seen cwd is the first
                    # LINE's cwd for JSONL — the Claude Code case). NOTE: pre-order still reads a parent's
                    # own cwd before descending, so for a nested single-doc a shallow cwd can precede a
                    # textually-earlier nested one; deterministic either way (leg-1 review — comment fix).
                    stack.extend(reversed([v for v in cur.values() if isinstance(v, (dict, list))]))
                elif isinstance(cur, list):
                    stack.extend(reversed([x for x in cur if isinstance(x, (dict, list))]))

        # Whole-document first (pretty-printed single doc / JSON array of turns); for JSONL this
        # fails fast → line-by-line, in FILE ORDER so first-seen cwd is the first line's (ai_session shape).
        s = text.strip()
        parsed = False
        if s[:1] in "{[":
            try:
                visit(json.loads(s)); parsed = True
            except (json.JSONDecodeError, ValueError, RecursionError):
                pass
        if not parsed:
            for line in text.split("\n"):
                ...
                try:
                    ...
                    continue
                visit(obj)
                if seen[0] >= CHATLOG_AXES_MAX_NODES:
                    break

        return {
            "first_timestamp": _canonical_iso_ms(state["min"]) if state["min"] is not None else None,
            "last_timestamp": _canonical_iso_ms(state["max"]) if state["max"] is not None else None,
            "cwd": state["cwd"],
        }
    ...
    def _extract_chatlog_metadata(self, text: str, temporal_text: str | None = None) -> dict[str, Any] | None:
        ...
        return {
            ...
            # v1.34.0 (recall#62): session axes — flat top-level scalars, null when the log
            # doesn't carry them. Extracted from `temporal_text` when the caller passes the FULL
            # file (the scan path — so min/max span the whole session, not the 64 KB window); falls
            # back to `text` (unit calls / small logs where windowed == full).
            **self._extract_chatlog_temporal(temporal_text if temporal_text is not None else text),
        }
        ...
        for offset, pattern in constraints:
            if isinstance(offset, _Within):
                ...
                if pos < 0:
                    return None
                if enforce_window:
                    # v1.23.1: bounded find-anywhere (C1 sniff) — pattern must start in window.
                    if pos > offset.n:
                        return None
                    # v1.23.2: + structural corroboration — the sample must also carry one of
                    # the corroborating tokens, so a deep literal that merely contains the
                    # pattern (no real structure) is rejected even inside the widened window.
                    # C2 (enforce_window=False) skips this, so embedded-signature polyglots
                    # still register for is_polyglot.
...
def compute_manifest_checksum(manifest: ScanManifest, trusted_only: bool = False) -> str:
    ...
    v1.45 (#147): the human-readable ``summary`` is excluded (it's a derived view, not an
    observation) — a one-time change that moved every default checksum once (LOGIC 1.23.0).
    Default-mode checksums are NOT comparable across the v1.45 boundary; within a build they
    are deterministic and independent of the summary content."""
    d = asdict(manifest)
    d["manifest_checksum"] = ""
    ...
    d["meta"]["scan_id"] = ""
    d["meta"]["generated_at"] = ""
    # v1.45 (#147): the human-readable `summary` is a DERIVED VIEW of the sealed data (files/vectors/
    # quality) — not an observation. Exclude it so a prose refresh never moves the checksum (no LOGIC
    # bump for future summary work). Consequence: the summary is now a non-sealed, unsigned derived view
    # (the signature is computed from this checksum) — trust the sealed machine fields (PUBLIC_CONTRACT §1.8).
    d["summary"] = ""
    if trusted_only:
...
def scan(path: str | Path, *, specialists: bool = False, workers: int = 1,
         **config_kwargs: Any) -> ScanManifest:
    """v1.26: one-call convenience — scan ``path`` and return the manifest.

    Equivalent to ``Scanner(Path(path), ScannerConfig(enable_specialists=specialists,
...
# v1.42: screening-receipt projection (#133 + blah_mad's bridge id). A compact, audit-friendly,
# safe-to-feed-a-model view of the BUILT manifest + a per-file tamper-evident receipt_id. A
# projection (runs after manifest_checksum exists) → LOGIC/SCHEMA frozen (front-door); default
# manifest byte-identical. See PUBLIC_CONTRACT §1.14 + docs/v1.42.0_RFC_Specification.md.
RECEIPT_DOC_VERSION = 1


def _receipt_id(manifest_checksum: str, rel_path: str, checksum_sha256: str) -> str:
    """v1.42: tamper-evident per-file bridge id — sha256 over a LENGTH-PREFIXED canonical preimage
    of (manifest_checksum, rel_path, checksum_sha256), so no two distinct triples share a preimage
    (the v1.38 dictionary_id `|`-collision lesson). Independently recomputable → verifiable.
    Content change → checksum → id; scan change → manifest_checksum → id."""
    parts = (manifest_checksum, rel_path, checksum_sha256)
...
def _file_receipt(fr: dict[str, Any], manifest_checksum: str) -> dict[str, Any]:
    ...
    receipt: dict[str, Any] = {
        ...
        "path_id": sha256(path.encode("utf-8")).hexdigest() if path else None,
        "checksum_sha256": fr.get("checksum_sha256"),
        "size_bytes": fr.get("size_bytes"),
        "mime_type": fr.get("mime_type"),
        "safety_flags": fr.get("safety_flags") or [],
    }
    lx = (fr.get("specialist_metadata") or {}).get(LEXICON_NAMESPACE)
    if lx:   # v1.38 body + v1.41 metadata screening result, when a lexicon ran
        md = lx.get("metadata") or {}
        receipt["lexicon"] = {
            "body_hits": lx.get("total_hits", 0),
            "metadata_hits": md.get("total_hits", 0),
            "categories": {c: d.get("count", 0) for c, d in (lx.get("categories") or {}).items()},
            "metadata_categories": {c: d.get("count", 0) for c, d in (md.get("categories") or {}).items()},
        }
    return receipt


def manifest_to_receipt(manifest: ScanManifest) -> str:
    ...
    envelope = {
        ...
        "scanner_version": manifest.context.scanner_version,
        "logic_version": manifest.context.logic_version,
        "schema_version": manifest.schema_version,
        "manifest_checksum": mc,
        "manifest_signature": manifest.manifest_signature,
        "scan_id": manifest.meta.scan_id,
        "generated_at": manifest.meta.generated_at,
        "dictionary_id": (lexvec or {}).get("dictionary_id"),
        "receipts": [_file_receipt(fr, mc) for fr in files],
    }
...
def main() -> None:
    ...
    # v1.38.1 (#126): version probing — a downstream (recall's doctor) read the usage banner and
    # false-positived fo as "below floor". Prints `file-observer X.Y.Z` to stdout and exits 0.
    parser.add_argument("-V", "--version", action="version", version=f"file-observer {SCANNER_VERSION}",
                        help="Print the version (`file-observer X.Y.Z`) and exit.")
    ...
    parser.add_argument("--receipt", action="store_true", help="Emit a compact screening RECEIPT projection instead of the full manifest: an audit envelope (versions, manifest_checksum + signature, scan_id, dictionary_id) + a per-file record (receipt_id, path_id, checksum, size, mime, safety_flags, lexicon hit-summary). The `receipt_id` is a tamper-evident hash of (manifest_checksum + path + file hash) — the join key a downstream read/skip log references. fo never records the read/skip decision (that's the orchestrator's). Safe by construction (no raw path). A projection — LOGIC/SCHEMA frozen, default manifest byte-identical. (v1.42)")
    ...
    if args.schema:
        ...
        if args.schema_format == "json-schema":
            ...
            _write_stdout_utf8(manifest_json_schema_str() + "\n")
            return
        doc = build_schema_document()
        if args.schema_format == "md":
            _write_stdout_utf8(schema_to_markdown(doc) + "\n")
        elif args.schema_format == "summary":
            _write_stdout_utf8(schema_to_summary(doc) + "\n")
        else:
            _write_stdout_utf8(schema_to_json(doc) + "\n")
        return

    # v1.30 (#109): fail loud on bad input. A silently-accepted user error (a typo'd source,
    # --workers 0, a negative --preview-max) produces a SUCCESSFUL empty manifest — the worst
    # outcome for the stack fo sits under (trellis indexes nothing → recall answers "no
    # results", a silent cascade with nothing to triage). Validate BEFORE scanning; exit 2
    # (the established bad-invocation code, parity with the --stdout/--watch checks). An empty
    # REAL directory stays valid (0 files is an observation) — only nonexistent/non-dir errors.
    _src = Path(args.source)
    if not _src.exists():
        print(f"file-observer: source directory not found: {args.source}", file=sys.stderr)
        sys.exit(2)
    if not _src.is_dir():
        print(f"file-observer: source is not a directory: {args.source}", file=sys.stderr)
        sys.exit(2)
    if args.workers < 1:
        print("file-observer: --workers must be >= 1", file=sys.stderr)
        sys.exit(2)
    ...
    if args.receipt:
        # v1.42: the screening-receipt projection — a compact audit record + a per-file bridge
        # receipt_id. Its own projection (already safe / fo-derived), so it takes precedence over
        # --trusted-only / --format.
        output = manifest_to_receipt(manifest)
        ext = "receipt.json"
    ...
    # v1.30 (#110): default output to the cwd, NOT the installed package dir. Runtime data
    # MUST NOT be written into the code tree (breaks read-only/container installs, pollutes a
