...
class Scanner:
    ...
    def _build_summary(self, manifest: 'ScanManifest') -> str:
        ...
        # v1.45 (#147): AI-session logs — token-usage SUMS across the recognized token fields.
        # Counted, NEVER priced (the federation charter's bright line).
        _ai_token_keys = ("input_tokens", "output_tokens", "cache_read_tokens",
                          "cache_creation_tokens", "reasoning_tokens")
        ai_files = [r for r in manifest.files
                    if r.specialist_metadata and "ai_session" in r.specialist_metadata]
        if ai_files:
            tokens = 0
            for r in ai_files:
                usage = r.specialist_metadata["ai_session"].get("usage") or {}
                tokens += sum(usage.get(k) or 0 for k in _ai_token_keys)
            lines.append(f"AI sessions: {len(ai_files)} log{'s' if len(ai_files) != 1 else ''}, "
