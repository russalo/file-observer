...
def _should_prune_dir(path: Path, root_resolved: Path) -> bool:
    ...
    Prune when ``path`` is a reparse point (NTFS junction / directory symlink).
    When its attributes can't be read (inconclusive), FAIL CLOSED via a
    resolve()-containment fallback: prune if it resolves outside the tree or can't
    be resolved at all (an inaccessible dir yields nothing lost). A normal,
    readable in-tree directory returns False (descend). This closes the tree-escape
    AND the crafted-cycle / huge-target DoS (Codex P1) before rglob would have
    materialized the junction's subtree.
    """
    try:
        ...
        return bool(os.lstat(path).st_file_attributes & _REPARSE_ATTR)  # type: ignore[attr-defined]
