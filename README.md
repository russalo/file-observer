...
{
  "schema_version": "1.25",
  "context": { "scanner_version": "1.50.0", "logic_version": "1.26.0", "...": "…" },
  "files": [
    {
...
> **New here?** Walk through the [**tutorial**](https://github.com/russalo/file-observer/blob/main/docs/TUTORIAL.md) (first scan → pipeline integration) or run the [**examples**](https://github.com/russalo/file-observer/tree/main/examples/) — self-contained, one per concept.
...
| **License** | [AGPL-3.0](https://github.com/russalo/file-observer/blob/main/LICENSE) ([commercial license available](https://github.com/russalo/file-observer/blob/main/LICENSING.md)) |
...
The image bundles `libmagic` + all optional specialists. (Builds from the [`Dockerfile`](https://github.com/russalo/file-observer/blob/main/Dockerfile); published to GHCR on each release.) Mount your data **read-only** and keep the output file outside the scanned tree, so a manifest you redirect into the same folder isn't picked up by a later scan.

**GitHub Action** — scan a repo in CI and capture the manifest as an artifact:

```yaml
- uses: russalo/file-observer@v1.50.0     # pin a release tag
  id: scan
  with:
...
It **observes and reports** — it never judges whether a file is safe; the agent interprets. Server-startup flags: `--root <dir>` (restrict scans to a subtree), `--lexicon`/`--lexicon-index` (apply a content screen), `--trusted-only` (force safe mode). Per-call tool params mirror the CLI safe surfaces — `trusted_only=true`, `receipt=true`, and `previous_manifest_path` for a delta. See [`examples/08-mcp-server/`](https://github.com/russalo/file-observer/tree/main/examples/08-mcp-server/).

**Treat the returned manifest as untrusted data.** The scanner can't be prompt-injected, but the manifest *reports* attacker-controllable strings verbatim — filenames, `content_preview`, embedded metadata — so a consumer must treat those file-derived fields as data, not instructions. Prefer the fo-derived signal (counts, `safety_flags`) over the verbatim strings. Those flags are routing signals, not a verdict — an *absent* flag means "nothing observed," never "safe," so unflagged files stay untrusted. You can still route on the signal: send *flagged* files to stricter (blocking) review and the rest through a lighter (advisory) path, rather than an all-or-nothing gate. Sensitive config (the `--lexicon` terms) is passed to the server at startup, never as a tool argument, so it never enters the agent's context ([why](https://github.com/russalo/file-observer/blob/main/SECURITY.md#mcp-never-pass-secrets-as-tool-arguments)). Full detail on the trust boundary in [SECURITY.md](https://github.com/russalo/file-observer/blob/main/SECURITY.md#the-manifest-is-untrusted-data).
...
The lexicon and receipt are the *detect* and *audit* ends of the "consume untrusted files safely" arc (safe mode below is the *hand-off*). See [`examples/10-lexicon-screen/`](https://github.com/russalo/file-observer/tree/main/examples/10-lexicon-screen/) and the [tutorial](https://github.com/russalo/file-observer/blob/main/docs/TUTORIAL.md#12-content-signals-bring-your-own-lexicon).
...
Every field is labeled `fo_derived` (trusted) vs `file_derived` (attacker-controllable) in `fo --schema`, so you can also build your own projection. See [`examples/09-trusted-only/`](https://github.com/russalo/file-observer/tree/main/examples/09-trusted-only/) and the [tutorial](https://github.com/russalo/file-observer/blob/main/docs/TUTORIAL.md#11-safe-mode-feeding-untrusted-files-to-a-model).
