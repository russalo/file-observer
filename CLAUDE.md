...
- `docs/v1.3.0_RFC_Specification.md` — **current release spec**. Pure-Python content-based MIME fallback (no libmagic): `detect_mime` gains a magic-signature tier (`_sniff_mime`) between libmagic and the extension fallback; `MAGIC_SIGNATURES` generalized to a multi-constraint matcher + expanded to ~24 formats incl. RIFF→WebP/WAV/AVI disambiguation (no more `riff_container`). New provenance trigger `magic_signature_fallback`. SCANNER 1.3.0; **LOGIC 1.1.4→1.2.0**; SCHEMA 1.2 (unchanged). Net current: **LOGIC 1.2.0, chatlog method_version 8**, SCHEMA 1.2.
...
- **v1.3.0 (current; shipped & live on PyPI):** Pure-Python content-based MIME fallback — `detect_mime` gains a magic-signature tier between libmagic and the extension fallback, so no-libmagic environments get content MIME for ~24 formats. `MAGIC_SIGNATURES` generalized to a multi-constraint matcher + expanded (archives/images/data/executables/media + RIFF→WebP/WAV/AVI; `riff_container` removed). New provenance trigger `magic_signature_fallback`. Enriches `file_signature`/`format_signatures`/`is_polyglot` too. SCANNER 1.3.0; **LOGIC 1.1.4→1.2.0**; SCHEMA 1.2 (unchanged). The data-gated word-twisting study moves to a later minor.
...
Test suite: 654 tests across `test_unit.py`, `test_integration.py`, `test_golden.py`, `test_edge_cases.py`, `test_packaging.py`, `test_v1_1.py`, `test_v1_2.py`, `test_v1_2_1.py`, `test_v1_2_2.py`, `test_v1_2_3.py`, `test_v1_2_4.py`, `test_v1_3.py` (as of v1.3.0).
