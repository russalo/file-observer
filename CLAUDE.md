...
- **v1.5.0 (current; built, in review):** PDF specialist head+tail read + `requires_vision` fix — surfaced by the infra-standards corpus (the corpus taught us about the observer). The PDF specialist read only the 8 KB head; page tree/`/Info`/font markers live in the trailer + compressed streams, so `page_count`/`producer` were null and born-digital PDFs were mis-flagged `requires_vision` (same bug class as pre-v0.7.1 OLE2). Fix (stdlib, no new dep): read head + bounded 128 KB tail; new provisional `pdf.text_detected`; `requires_vision` only when no-text AND image markers. Validated on 964 real PDFs: **requires_vision 310→12, page_count 9→341, producer 9→308**; object-stream PDFs return `page_count=None` (honest null). SCANNER 1.4.0→1.5.0; **LOGIC 1.3.0→1.4.0**; SCHEMA 1.3→1.4 (additive). Falsify-first; reviewed.
...
- **Next 3 minors (planned 2026-06-03, from the v1.5 PDF-work learnings — see [[infra-standards-corpus-and-pdf-specialist-finding]]):**
  - **v1.6.0 — production-provenance dimension** (#3, the meatiest). Elevate *production provenance* — toolchain (producer/creator), production era (creation_date), digitization origin (born-digital / scanned / OCR'd) — to a first-class **corpus-scoped `provenance` vector**, modeled on `author_aggregate` (which it complements: that's WHO authored; this is WHAT-TOOL / WHEN / digitization). Closed, documented producer-normalization table; reuses v1.5 `requires_vision`/`text_detected` + OCR-producer fingerprints for digitization. Pure observation — the Wayne-K facts (toolchain across jurisdictions, OCR'd "standards", production era) were ALL provenance; this is the scanner's strongest/most novel seam and squares with the "deterministic Tika **+ provenance**" positioning. RFC drafting now.
