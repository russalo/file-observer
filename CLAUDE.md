...
- **v1.6.0 (current; built, in review):** production-provenance dimension — a corpus-scoped **`provenance` vector** (models on `author_aggregate`; complements it — that's WHO authored, this is WHAT-TOOL / WHEN / digitization): normalized `toolchains` (closed table — Adobe PDF Library, Autodesk HDI, Bluebeam, Microsoft, TeX, OCR fingerprints…), `production_years` (creation_date), `digitization` (born_digital/scanned/**ocr_detected**, reusing v1.5 `requires_vision`+`text_detected` + OCR-producer fingerprints). Cross-format: PDF producer + OOXML `app.xml` `<Application>`. **Pure aggregation — LOGIC unchanged (1.4.0).** Validated on `corpora_infra` (reproduced the Wayne-K facts); the re-scan also caught + fixed two extraction bugs (UTF-16BE literal `/Info` → shared `_decode_pdf_bytes`; messy version-suffix strip) — falsify-first earning its keep again. SCANNER 1.5.0→1.6.0; SCHEMA 1.4→1.5 (additive vector); `provenance` method_version 1. Net current: **LOGIC 1.4.0, SCHEMA 1.5**, chatlog method_version 9, provenance method_version 1.
