...
- `docs/v1.4.0_RFC_Specification.md` — **current release spec**. Content-shape chatlog detection gate, ADDITIVE over the retained v1.2.4 machinery (the non-speaker stop-list is load-bearing — falsification proved the draft's "replace it" premise wrong). The prose rule gains `utterance_ratio≥0.6` — a turn is an *utterance* when it has a **function word** (the discriminator: natural turns have them, atomic data values don't), ends in sentence punctuation, is multi-word, or is long — rejecting cyclic data tables (`Item:`/`Price:`). Plus FP-lexicon dominance (`Added:` changelogs), density floor (sprinkled labels), version/date structure vote-against (release notes), FAQ complete-set exclusion (`{question,answer,q,a,faq}`; `A:`/`B:` survives). New provisional `content_shape` (`utterance_ratio`/`density`). On real data == v1.2.4 (0 diff/3,032 files); changes only adversarial inputs. Accepted FNs: ultra-terse contentless dialogue, all-distinct roll-call, `Q:`/`A:` interviews. SCANNER 1.3.0→1.4.0; **LOGIC 1.2.0→1.3.0**; SCHEMA 1.2→1.3 (additive). Net current: **LOGIC 1.3.0, chatlog method_version 9**, SCHEMA 1.3.
