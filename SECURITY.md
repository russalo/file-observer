...
The scanner's attack surface is limited to parsing. A malformed file can cause a specialist to raise an exception or return incorrect metadata, but cannot cause code execution.
...
| `defusedxml` | XML parsing | Replaces stdlib `xml.etree` to block entity expansion attacks. Stdlib fallback used when unavailable (documented risk). |
