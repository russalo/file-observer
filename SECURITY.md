...
| `pypdf` (`[pdf]` extra, v1.8) | Object-stream PDF page_count / `/Info` | Tier-1 decode path. Stdlib in-house decoder is the tier-2 fallback when absent (zlib + PNG predictor + `/W` xref + `/ObjStm`, scoped to common cases — never returns a wrong value, only null on exotic predictors). Both tiers operate within `PDF_INFLATE_CAP` (64 MB output cap; v1.8.1 hardening). |
...
| 1.0–1.9.x | No (schema-stable but unsupported; please upgrade) |
| < 1.0 | No |
