...
python3 -c "import json;m=json.load(open('out/manifest.json'));v=[x for x in m['vectors_collected'] if x['vector_id']=='lexicon'];print(json.dumps(v[0]['summary'],indent=2) if v else 'no lexicon vector')"

echo
echo "--- files a consumer would route to review (lexicon_match flag) ---"
python3 -c "import json;m=json.load(open('out/manifest.json'));[print(' ',f['path'],'->',f.get('safety_flags')) for f in m['files'] if 'lexicon_match' in (f.get('safety_flags') or [])]"
...
print("  lexicon-derived fields carry any term?  ", leaked or "NO — only counts, category names, and a dictionary_id")
