...
| 1.3.0 | 1.2 | 2026-06-02 | **Pure-Python MIME fallback (no libmagic) + RIFF disambiguation.** `detect_mime` gains a content-based magic-signature tier between libmagic and the extension fallback, so no-libmagic environments (Windows, minimal containers) get content-derived MIME for ~24 common formats instead of extension-only. `MAGIC_SIGNATURES` generalized to a multi-constraint `(constraints, mime)` matcher (shared `_signature_matches` helper feeds both the new `_sniff_mime` and `scan_signatures`) and expanded: archives (gzip/bzip2/xz/7z/zstd/RAR), images/data (TIFF/BMP/SQLite/Parquet), executables (ELF/PE), media (MP4/Matroska/MP3/FLAC/OGG), and **RIFF disambiguation** into WebP/WAV/AVI via an offset-8 marker (the non-MIME `riff_container` label is gone — a WAV now matches exactly one signature, no spurious polyglot). New provenance trigger `magic_signature_fallback`. Side benefit: `file_signature`/`format_signatures`/`is_polyglot` are enriched by the same table (verified no golden impact — goldened fixtures all use pre-existing signatures). SCANNER 1.2.4→1.3.0; **LOGIC 1.1.4→1.2.0** (new MIME routing); SCHEMA unchanged (1.2). 654 tests. RFC-driven, cross-model + multi-agent reviewed. | [v1.3.0_RFC_Specification.md](v1.3.0_RFC_Specification.md) | [COMPLIANCE-v1.3.md](COMPLIANCE-v1.3.md) |
