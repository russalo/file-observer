...
reparse tags, since v1.46); and a **named allowlist** of documented origin attributes
(v1.48).

fo does **not** enumerate open metadata namespaces, and does not read a channel merely
because it is readable. An attribute joins the allowlist only if it has documented fixed
semantics, is bounded, concerns origin or integrity rather than user annotation, and can
be read by name rather than by enumeration.

This states a boundary fo has exercised since v0.3 rather than granting a new one; it was
written down at v1.48 when the first *open* channel (extended attributes) was admitted.
...
- `origin` — the entire block (`downloaded`, `zone`, `agent`, `acquired_at`, `source`) is **provisional** (new in v1.48, download-origin observation). Always present; **`null` when no marker was read**. **Absence is not evidence** the file was not downloaded — extended attributes and alternate data streams are lost by ordinary `cp`/`rsync`/`tar`/`zip`, so `origin` describes the file *as it exists on this filesystem*, not its history. `agent` is the one `file_derived` leaf (free text written by whatever fetched the file) and nulls under `--trusted-only`; the rest are fo_derived and survive, so safe mode still learns *that* the file came from outside.
...
- `specialist_metadata.chatlog.first_timestamp`, `.last_timestamp`, `.cwd` — session time axis (min/max turn timestamp → canonical ISO-8601 UTC; null when untimestamped) + first-seen working directory (verbatim/bounded; null when absent) (provisional since v1.34)
...
- `specialist_metadata.ai_session.*` — the entire `ai_session` namespace (`vendor`, `surface`, `models`, `id_prefix`, `object_types`, `schema_mismatch`, `usage` [token-usage sums], `usage_by_model`) is **provisional** (new in v1.33, AI-session observation; `usage_by_model` added v1.35). Present only on `is_chatlog`-detected AI session logs (Claude Code / OpenAI / Gemini). Token-usage sums are **observe-only — counted, never priced** (the federation bright line). The namespace *key* is additive-stable (§1.4); its fields are provisional.
...
**Promoted to stable in v1.47.0** (now under the backward-compat policy — not removable/retypable without a MAJOR bump): the **`presentation`** namespace (`slide_count`/`title`/`author`/`application`) AND the **`audio`** namespace (`format`/`bitrate`/`duration_s`/`title`/`artist`/`album`/`year`). Settled logic since ship (presentation v1.24 via `.pptx`/`.odp` + legacy `.ppt` OLE2; audio v1.25 via `.mp3` — extraction unchanged since ship; v1.25.1 touched provenance *accuracy*, not values), corpus-swept, and hardened (presentation reuses the docx/xlsx ZIP+XML + OLE2 `SummaryInformation` machinery already stable since v1.14; audio was the lone net-new binary parser carried at the v1.8.1 never-crash/bounded bar from commit one, folded into `test_capture_metadata_hardening`). These are exactly the fields v1.31.0 deferred as "too young, season next pass". (`.odt` adds nothing new to `document`; `.ods` nothing to `spreadsheet`; `.jp2`/`.tiff` reuse the stable `image` fields.) **Observed field VALUES byte-identical** (designation-only — stability lives only in `--schema`, never the manifest); `manifest_checksum` moves for every manifest on v1.47.0 only because `schema_version` (1.23→1.24) is in the checksum preimage, as on any SCHEMA bump, while every extracted value is unchanged; `LOGIC_VERSION` unchanged (1.24.6). SCHEMA 1.23→1.24 (promotion = contract change, the v0.11/v1.10/v1.14/v1.23/v1.31 precedent).
...
| `1.25` | 1.50.0 | **Public-face pass (minor) — no manifest change.** The deprecated `scanner` import shim (v1.0.1) is **removed**: `import scanner` no longer resolves to fo (the import path was never under this contract — v1.0.1 precedent — but its removal is called out here so no consumer is surprised; the canonical `import file_observer` has been the documented path for 80 releases). Packaging/metadata: `py.typed` shipped (`Typing :: Typed` now true), PEP 639 SPDX `license` + `license-files` (the `LICENSE` file is the verbatim AGPL-3.0; dual-licensing terms in `LICENSING.md`), Python 3.14 classifier + CI coverage for 3.13/3.14, the sdist no longer carries an unrunnable `tests/` tree, `python -m file_observer`, README links absolute so the PyPI page works. `LOGIC_VERSION` 1.26.0 / `SCHEMA_VERSION` 1.25 **frozen**; every extracted VALUE unchanged (`manifest_checksum` moves only via `context.scanner_version`, as on any release). No existing field changed/removed/retyped. |
| `1.25` | 1.49.0 | **Chatlog: the JSON message floor becomes conditional on content shape (minor).** A session of **two** message objects now detects when the roles are recognised conversational values AND every turn's content is utterance-shaped — the floor is not lowered, it is qualified. Grounded on **LongMemEval**: detection is 99.3% with exact turn counts and the entire residual was this floor (0/7 at two turns, 100% at ≥3); recall's 5.9% miss at 19,195 files is the same tail. **Strictly ADDITIVE** — `is_chatlog` moves False→True only (the v1.29 precedent), so no consumer loses a detection. chatlog `method_version` 11→12; `LOGIC_VERSION` 1.25.0→1.26.0; `SCHEMA_VERSION` **1.25 frozen** — no field added or retyped. Two further changes were specified in the RFC and **falsified during implementation** (see §3.1/§3.2 there); they are recorded as documented residuals with the measurements that killed them rather than shipped. |
| `1.25` | 1.48.1 | **MCP SDK 1.x + 2.x support (#174) — packaging/compat only, no manifest change.** mcp 2.0.0 renamed `mcp.server.fastmcp.FastMCP` to `mcp.server.mcpserver.MCPServer`; verified against a real 2.0.0 install that this is a RENAME, not a redesign (constructor kwargs, `run(transport=...)`, and the `.tool()` decorator are unchanged), so one aliased import covers both SDKs and all four v1.37 tools register identically. The `[mcp]` extra moves `>=1.0,<2` → `>=1.0,<3`; `[security]`'s `purexml` gains its missing `<2`. A packaging test now asserts every extra bounds its upper major — an unbounded `>=` is how mcp 2.0 broke four CI jobs on a branch that touched no MCP code. `LOGIC_VERSION` 1.25.0 / `SCHEMA_VERSION` 1.25 both **frozen**, and no manifest field is changed/removed/retyped. **Precision (the v1.47.0 lesson):** "frozen" means every extracted VALUE is unchanged — verified across a 964-file corpus, every top-level key identical — NOT that `manifest_checksum` is unchanged. The checksum moves on any version bump, patch included, because `context.scanner_version` sits inside the preimage (Pillar 1: the ScanContext is the capability fingerprint, and context is what explains variance). A consumer pinning the checksum re-baselines on every release; a consumer keying on values sees nothing move. |
| `1.25` | 1.48.0 | **Download-origin observation (xattr / ADS) — new provisional `origin` block.** Records whether a file *arrived from outside this machine*, read from a **named allowlist** of documented origin attributes (`com.apple.quarantine`, Windows `Zone.Identifier`, `user.xdg.origin.url`) — fo never enumerates an extended-attribute namespace. Fields: `downloaded`/`zone`/`agent`/`acquired_at`/`source`. **The origin URL is never surfaced** in any form (`kMDItemWhereFroms`, `ReferrerUrl`, `HostUrl`) — it was measured at 0.36% coverage AND is attacker-controllable free text, the class §1.13 safe mode exists to contain; the same line fo draws at `gps_present`. `origin` is **always present, `null` when no marker was read** (Pillar 4: null means not-observed). Because the key is always present it enters every checksum preimage → **every manifest's `manifest_checksum` moves once**, ungated (the v1.36/v1.45 class); values change only where a marker exists. `LOGIC_VERSION` 1.24.6→1.25.0. No existing field changed/removed/retyped. |
| `1.24` | 1.47.1 | **Polish pass: static-analysis + doc-consistency hygiene (patch, behavior byte-identical, LOGIC/SCHEMA frozen).** No manifest field/value/checksum change. **Doc-accuracy fixes to THIS contract:** §2.4 now marks `presentation`/`audio` **stable** (the v1.47.0 promotion updated §3 but not the §2.4 stability prose — the §2.4 designations were unguarded) and adds the `ai_session` namespace + the chatlog session axes (`first_timestamp`/`last_timestamp`/`cwd`) to the provisional list (they were provisional in code but undocumented as such); a v1.31.0→v1.23.0 copy-paste typo in the SCHEMA-1.14 row is corrected. **Operationalized guards** (so the drift class can't recur): a `§2.4 ↔ PROVISIONAL_SPECIALIST_FIELDS` stability-consistency test, a namespace-documentation-completeness test, a SECURITY-supported-versions test, a SCHEMA quick-ref test, and a `ruff` static-analysis CI gate. Also SECURITY.md's Supported-Versions table refreshed; the per-release `test_version_axes` version-pin duplication (16 files) consolidated to one gate. `LOGIC_VERSION` 1.24.6 / `SCHEMA_VERSION` 1.24 both **frozen**. No existing field changed/removed/retyped. |
| `1.24` | 1.47.0 | **Promotion pass: the `presentation` + `audio` namespaces → stable (minor, designation-only).** The third promotion pass (after v0.11/v1.10/v1.14/v1.23/v1.31), and exactly the pair v1.31.0 deferred as *"presentation/audio — too young, season next pass."* Now ~3 weeks seasoned (the v1.31 bar): the `presentation` namespace (`slide_count`/`title`/`author`/`application`, v1.24) and the `audio` namespace (`format`/`bitrate`/`duration_s`/`title`/`artist`/`album`/`year`, v1.25) graduate provisional → stable — settled logic (extraction unchanged since ship; v1.25.1's OLE2 patch changed provenance *accuracy*, not any extracted value), corpus-swept every release, and hardened (presentation reuses the docx/xlsx ZIP+XML + OLE2 machinery; audio was the lone net-new binary parser carried at the v1.8.1 bounded bar + folded into `test_capture_metadata_hardening`). **Every extracted VALUE is byte-identical** (designation-only — stability lives only in `--schema`, never the manifest) — but because `schema_version` is in the checksum preimage, `manifest_checksum` moves for every manifest on v1.47.0 (as on any SCHEMA bump), so a consumer pinning the checksum re-baselines once while every value is unchanged; `LOGIC_VERSION` unchanged (1.24.6). **HELD:** the `chatlog` family (alpha-locked + the parked non-count redesign), `fact_block`/`ai_session`/`lexicon_match` (too young — the data-gathering tier), `format_signatures`/`is_polyglot` (held-by-design, §2.4). SCHEMA 1.23→1.24 (promotion = contract change, the v0.11/v1.10/v1.14/v1.23/v1.31 precedent). No existing field changed/removed/retyped. |
| `1.23` | 1.46.8 | **Windows reparse-point pruning gates on the name-surrogate tag bit, not the bare reparse bit — fixes a silent-omission (patch, SCHEMA unchanged, LOGIC value-change, Windows-only).** The v1.46.0 Windows discovery containment (`_should_prune_dir`) pruned any directory whose `st_file_attributes` had `FILE_ATTRIBUTE_REPARSE_POINT` set — a **superset** of junctions/dir-symlinks that also flags **OneDrive Files-On-Demand, Data-Dedup, and AppExecLink** dirs. Those in-tree subtrees were then **silently dropped from `files[]` with no `ErrorRecord`** (an incomplete manifest that looked complete; confirmed on native NTFS). Now prunes **only name surrogates** — junctions + directory symlinks (`st_reparse_tag & 0x20000000`, the documented `IsReparseTagNameSurrogate` predicate; the `IO_REPARSE_TAG_*` constants aren't in stdlib, and the surrogate bit future-proofs against any new class) — so non-surrogate reparse dirs are **descended** (the file-level `resolve()`-containment still catches any genuine escape; junction/symlink containment is unchanged). **Windows-only file-set change** (a previously-dropped in-tree subtree reappears); **Linux/macOS byte-identical** (`_should_prune_dir` is called only under `elif _WINDOWS`), so `manifest_checksum` moves only on Windows scans of trees containing a non-surrogate reparse dir. `LOGIC_VERSION` 1.24.5→1.24.6 (the v1.46.0/v1.15 cross-platform-LOGIC precedent — the observing logic changed even though Linux bytes are unchanged); `SCHEMA_VERSION` unchanged (1.23). No field changed/removed/retyped. Validated on native NTFS (Tailnet); leg-1/leg-3/leg-4 reviewed. |
| `1.23` | 1.46.7 | **MIME synonym classes: a mismatch now means a genuine content-vs-extension disagreement, not two spellings of one format (patch, SCHEMA unchanged, LOGIC value-change).** `analyze_mime`'s content-vs-extension check was a strict `detected_mime == extension_mime`, so a legitimate synonym — most visibly the JSON family, where libmagic types a `.jsonl` as `application/x-ndjson` (multi-line) or `application/json` (single-line) while the extension map registers `application/jsonl` — was reported as a **mismatch**, tipping the file into `quality: degraded`. Surfaced by a consumer (recall) whose valid jsonl chatlog corpus scored **0 clean / 19,195 degraded** — every file a false mismatch, all JSON-family synonyms. `_mime_labels_compatible` now folds SAME-format-different-label classes (JSON `{application/json, application/jsonl, application/x-ndjson}`, plus `text/xml`≡`application/xml`, `text/rtf`≡`application/rtf`, `text/markdown`≡`text/x-markdown`, and the two Python-source/bytecode pairs), so `matches_extension` flags only a real disagreement (a `.txt` that is really a PNG still flags). **Deliberately NOT folded:** supertype relationships — a `text/plain` sniff of a text-based format like markdown/yaml — which is a separate, broader question left untouched. `matches_extension` moves **False→True** on synonym-labelled files, so `manifest_checksum` moves for corpora containing them (jsonl/json/xml/rtf/python), while default corpora are byte-identical (the v1.25.1 accuracy-fix precedent). `LOGIC_VERSION` 1.24.4→1.24.5; `SCHEMA_VERSION` unchanged (1.23) — `matches_extension` is an existing boolean, now more accurate. No field changed/removed/retyped. |
