...
| `1.7` | 1.8.2 | **Patch (determinism fix), schema unchanged.** No manifest-shape change. Behavior: a file that fails `stat()` (e.g. deleted mid-scan, or a TOCTOU race between discovery and read) now reports `modified_at: null` instead of the wall-clock scan time — so its degraded `FileRecord` (and therefore `manifest_checksum`) is reproducible run-to-run. Matches the `created_at: null` already on that record. `LOGIC_VERSION` 1.4.1→1.4.2. No existing field changed/removed/retyped (`modified_at` was already nullable). |
