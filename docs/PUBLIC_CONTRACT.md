...
- `specialist_metadata.pdf.text_detected` — bool, whether `/Font`/text markers were seen in the PDF head+tail (the born-digital-vs-image signal; provisional since v1.5)
- `vectors_collected[]` entry `provenance` — corpus-scoped production provenance (normalized `toolchains`, `production_years`, `digitization`); also adds `application` to docx/xlsx specialist metadata (provisional since v1.6)
- `specialist_metadata.pdf.xref_type` — `classic` / `stream` / `none`, the PDF cross-reference form observed when following the structural anchor (provisional since v1.7)
...
| `1.6` | 1.7.0 | Structural-anchor PDF reader (additive): `specialist_metadata.pdf.xref_type` (`classic`/`stream`/`none`, provisional) + a new `signal_provenance.trigger` value `structural_anchor`. `page_count`/`/Info` are now read by following the PDF's index (`startxref` → trailer → root → page tree, parsing the xref table + `/Prev`) — precise on incremental updates and > 64 MB files; on the corpus, identical values to v1.5 (zero regression). Specialist extraction only — `LOGIC_VERSION` unchanged (1.4.0). No existing field changed/removed/retyped. |
