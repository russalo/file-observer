...
### 1.2 LOGIC_VERSION
**What it is:** The version of the observing logic — both the code that decides routing (`is_binary`, `requires_vision`, `requires_specialist_tool`, the SPECIALIST_TOOLS dict, SUPPORTED_EXTENSIONS, SPECIALIST_NAMESPACE) AND the code that derives the values it writes to the manifest.
**Where it lives:** `LOGIC_VERSION` constant in `src/file_observer/scanner.py`, `ScanContext.logic_version` in every manifest.
**When it bumps:** Any time the same file would produce **different observable manifest output** than before — a routing flag flips, OR a derived/observed value changes (a "values-move"; see the history below, which is mostly values-moves). It is the fingerprint of the whole observing logic, not routing alone.
**Format:** `MAJOR.MINOR.PATCH`. May lag SCANNER_VERSION.
**Current:** `1.26.0`  (1.25.0→1.26.0 at v1.49.0 — chatlog JSON floor made CONDITIONAL on content shape (two-object sessions with conversational roles + utterance content); strictly ADDITIVE (False→True only, v1.29 precedent), chatlog mv 11→12; 1.24.6→1.25.0 at v1.48.0 — download-origin observation: a new `origin` block on every FileRecord (null when no marker was read), read from a named allowlist of documented origin attributes; a NEW OBSERVATION, and because the key is always present it enters every checksum preimage → EVERY manifest's checksum moves once (ungated, the v1.15 ScanContext / v1.36 purexml-dep-record / v1.45 summary-exclusion class); values change only where a marker exists; no routing flag flips; prior UNCHANGED at v1.47.0 — the promotion pass (presentation + audio → stable) is designation-only; stability lives only in `--schema`, no observing logic changed, so LOGIC stays frozen (the v1.31 promotion precedent); 1.24.5→1.24.6 at v1.46.8 — Windows reparse-prune gates on the NAME-SURROGATE tag bit (`st_reparse_tag & 0x20000000`), not the reparse BIT (#169): a non-surrogate reparse dir (OneDrive Files-On-Demand / Data-Dedup / AppExecLink) is now DESCENDED instead of pruned → its in-tree subtree reappears in `files[]` on Windows; junctions/dir-symlinks still prune (containment intact, Tailnet-validated on native NTFS); a Windows-only file-SET change, Linux byte-identical (`_should_prune_dir` runs only under `elif _WINDOWS`) → the v1.46.0/v1.15 cross-platform-LOGIC precedent; 1.24.4→1.24.5 at v1.46.7 — MIME synonym classes: `analyze_mime` folds SAME-format-different-label pairs (JSON family application/jsonl≡x-ndjson≡json + xml/rtf/markdown/python) so `matches_extension` flags only a genuine content-vs-extension disagreement; a values-move — `matches_extension` False→True on synonym-labelled files → `manifest_checksum` moves for those corpora only (the v1.25.1 accuracy-fix precedent; default corpora byte-identical); 1.24.3→1.24.4 at v1.46.5 — never-crash EXIF type-gate (bestiary soak #165): a make/model tag with a numeric TIFF type is now `None` (the field stays `str|None`, never an `int` — an int crashed `_build_summary`'s `.strip()` at scan-abort scope); a value changes ONLY on a malformed-EXIF file (int→None) → `manifest_checksum` moves for those only (the v1.8.2/v1.25.1 degraded-path precedent; real corpora → NO-DRIFT); 1.24.2→1.24.3 at v1.46.4 — recognize macOS-universal metadata (`.DS_Store`=Bud1, `._*`=AppleDouble) by magic: `unsupported_extension` flips False + `format_signatures` gains the label for these files (a magic match is a positive content ID even when libmagic types them octet-stream; a genuine unknown still flags) → `manifest_checksum` moves for `.DS_Store`/AppleDouble-bearing manifests only (the v1.22/v1.23.3 recognition-change precedent; corpora without them → NO-DRIFT); 1.24.1→1.24.2 at v1.46.2 — pin `.vx → text/plain` (Windows shakedown MIME-sweep finding): `.vx` no longer flips `is_binary` on the no-libmagic Windows path; `extension_mime` None→text/plain + is_binary/matches_extension move for `.vx`-bearing manifests (a values-move, the v1.46 `.csv`-pin class; no `.vx` in any corpus → sweep NO-DRIFT); 1.24.0→1.24.1 at v1.46.1 (#156) — the `structural.csv_headers` signal_provenance entry is now emitted (new trigger `csv_header_row`); a values-move — `manifest_checksum` moves for CSV-with-headers manifests, no new manifest field, no routing flag flip (the v1.25.1 provenance-accuracy-patch precedent); 1.23.0→1.24.0 at v1.46.0 — native-Windows/NTFS hardening (A+B): `iter_files` reparse-aware containment (enumerate with `os.walk` and PRUNE reparse-point dirs before descent, Windows-only, fail-closed via `resolve()`-containment on an inconclusive attribute read; the v1.8.1 file-symlink containment retained) + `.csv` pinned to `text/csv`; both are cross-platform routing-LOGIC changes — Linux real-corpus output is BYTE-IDENTICAL (A skips only out-of-tree-resolving files, which v1.8.1 already skipped for file symlinks; B is a no-op on Linux) → sweep NO-DRIFT; the new behavior is Windows-only (junction escape closed; CSV no longer mistyped binary); the v1.15 cross-platform-LOGIC precedent; 1.22.0→1.23.0 at v1.45.0 — ONE-TIME: the human-readable `summary` is excluded from `manifest_checksum` (a derived view, not an observation), so EVERY manifest's checksum moves once (#147); ungated, the v1.15 ScanContext / v1.36 purexml-dep-record every-checksum-moves precedent; after this, summary refreshes are LOGIC-free; 1.21.0→1.22.0 at v1.41.0 — lexicon self-sweep over file-derived metadata: the new `lexicon_match.metadata` sub-block moves `manifest_checksum` only on lexicon scans, no routing flip; 1.20.0→1.21.0 at v1.38.0 — bring-your-own-lexicon term observer: a new baseline-tier content derivation (per-category term counts + density on a consumer-supplied lexicon); manifest_checksum moves ONLY for lexicon-supplied scans (the v1.30 gated-feature precedent), no routing flip; 1.19.0→1.20.0 at v1.36.0 — defusedxml→purexml XML-dependency changeover: fo opts into purexml's structural caps (RECOMMENDED_LIMITS: max_depth=1000/max_attributes=256/max_bytes=100 MiB) → rejects pathologically-deep/oversized XML defusedxml parsed (output changes on PATHOLOGICAL input only, the v1.30.2 precedent; parse byte-identical on real files); ALSO ScanContext.dependencies records purexml in place of defusedxml → manifest_checksum moves on EVERY manifest (dep record is in every ScanContext, Pillar 1 explains it); no routing flip; prior 1.18.0→1.19.0 at v1.35.0 — AI-session per-model usage attribution (increment 3): a new provisional `ai_session.usage_by_model` (per-model token-usage sums, keyed on the co-located model) — VALUES move `manifest_checksum` on ai_session corpora (the v1.29/v1.33 values-move precedent → LOGIC bump), no routing flip, ai_session method_version 1→2; prior 1.17.0→1.18.0 at v1.34.0 — chatlog session axes (recall#62): three new flat chatlog scalars (first_timestamp/last_timestamp = min/max turn timestamp normalized to canonical ISO-8601 UTC; cwd = first-seen) — new observed values move `manifest_checksum` on timestamped/cwd-bearing chatlog corpora (the v1.29/v1.33 values-move precedent → LOGIC bump), no routing flip, is_chatlog unchanged, chatlog method_version 10→11; prior 1.16.0→1.17.0 at v1.33.0 — AI-session observation increment 1: a new `ai_session` namespace on is_chatlog-detected AI session logs carries token-usage sums + a producer-schema fingerprint; VALUES move `manifest_checksum` on AI-session corpora (the v1.29 values-move precedent → LOGIC bump), no routing flag flips; prior 1.15.3→1.16.0 at v1.32.0 — new content-detection routing: the generic kv-fact-block specialist (FR #114) sets `is_fact_block` + a `fact_block` dispatch on a text body that is a `key: value` block (additive: False→True only); prior UNCHANGED at v1.31.0 — the capture-metadata promotion is designation-only, no routing/value change; 1.15.2→1.15.3 at v1.30.2 — ReDoS/bounded-time hardening, 3 content regexes made linear; 1.15.1→1.15.2 at v1.30.1 — self-inclusion skip anchored to the resolved output dir; 1.15.0→1.15.1 at v1.30.0 — discovery skips fo's own default output dir; 1.14.1→1.15.0 at v1.29.0 — chatlog detection recognizes agentic (tool-turn) sessions: a turn with a conversational role + a text-bearing block (backward-compat) OR a distinctive agentic block (`thinking`/`tool_use`/`tool_result`) counts in detection AND turn-counting signals → `is_chatlog` routes additively (False→True, strict superset) on tool-heavy logs + chatlog `method_version` 9→10 (generic `image`/`document` excluded — leg-1 review FP fix); prior 1.14.0→1.14.1 at v1.25.1 — OLE2 specialists (`.doc`/`.xls`/`.msg`/`.ppt`) declare a full-file deviation in `signal_provenance` (`ole2_full_file_required`) instead of the false `bounded_sample` → manifest-surface change, the v1.8.2/v1.9.1 precedent; 1.13.0→1.14.0 at v1.25.0 — audio (`.mp3`) + legacy presentation (`.ppt`) extraction → `requires_specialist_tool` routing change; 1.12.4→1.13.0 at v1.24.0 — office+image extraction adds specialists for `.pptx`/`.odp`/`.odt`/`.ods`/`.jp2`/`.tiff`/`.tif` → `requires_specialist_tool` routing change; 1.12.3→1.12.4 at v1.23.3 — bzip2 dual-magic + `_OneOf` matcher, recognizes empty bzip2; 1.12.2→1.12.3 at v1.23.2 — corroborated PDF-header sniff: `%PDF-` window 256→1024 + structural corroboration; prior 1.12.1→1.12.2 at v1.23.1 — PDF-header FP fix)
...
**Current vectors:**
- `chatlog` method_version: 12 (v1.49.0: the >=3 JSON message floor made CONDITIONAL on content shape — a two-object session qualifies when both normalized roles are recognized conversational values and every turn is utterance-shaped; the qualifying predicate is shared by detection AND extraction and feeds the rules_hash → 12; v1.34.0: session axes first_timestamp/last_timestamp/cwd feed the rules_hash → 11; v1.29.0: agentic tool-turn block sets → 10; v1.4.0: content-shape gate over the retained stop-list — `utterance_ratio≥0.6` via function-word/punctuation/length arms, FP-lexicon dominance, version-tag structure vote-against, FAQ complete-set exclusion; provisional `content_shape` surfaced — a density floor was prototyped then dropped in review, surfaced not gated. Prior: 8 = v1.2.4 case-insensitive stop-list + embedded-dialogue parity; 7 = v1.2.3 FAQ stop-list; 6 = v1.2.2 recurrence + stop-list; 5 = v1.2.1 distinct-speaker + co-signal; 4 = v1.2 generalized conversational JSON/JSONL; 3 = v0.10.1 JSONL role detection + v0.9.1 stop-list/H3 threshold)
...
|---|---|---|---|---|
| Package release | `SCANNER_VERSION` | `MAJOR.MINOR.PATCH` | 1.50.0 | Internal |
| Routing logic | `LOGIC_VERSION` | `MAJOR.MINOR.PATCH` | 1.26.0 | Internal* |
| Manifest shape | `SCHEMA_VERSION` | `MAJOR.MINOR` | 1.25 | **Public** |
...
|---|---|---|---|
| PDF markup annotations (`/Annots` count + subtype census) | needs a real markup-bearing specimen before a parser is written against a hypothesis rather than an encoding; and it is a BOUNDED-parse problem, not a field addition (one measured file carries **15,448** annotations on 89 pages — a naive walk is a cost and a DoS surface on untrusted input, so it earns the v1.8.1 bar from commit one) | **MEASURED 2026-08-07** (`scratch/pdf_markup_annotation_measurement_2026-08-07.md`): 654 real agency PDFs → **11 (1.7%)** carry markup annotations; producer-contains-Bluebeam **8/10 (80%)** vs **3/644 (0.47%)** otherwise = ~170× enrichment. Subtypes `/Square` 41,808 · `/Polygon` 20 · rest trivial. **⚠ NEGATIVE RESULT, measured after an earlier draft of this row overclaimed it:** of **34,932** `/Square` annotations examined, **0 carry `/Measure`** (the PDF measurement dictionary) and **0 carry `/IT`** — their keys are `/Rect /T /F /Contents /Border /NM`, i.e. plain rectangle markup with a note. So this corpus proves annotation DETECTION works and prices it, but contains **no measurement payloads at all** — it cannot validate the half of the candidate that matters. `/Square` being how Bluebeam renders area measurements does NOT make `/Square` evidence OF a measurement (the same claim-vs-observation error this candidate exists to avoid). `/Link`+`/Widget`+`/Popup` excluded as navigation/form chrome | a **measurement**-bearing Revu specimen supplied deliberately by CP (never rummaged from a consumer's corpus) — now the HARD blocker, since 34,932 measured annotations contained zero measurement dictionaries, so nobody here has seen how Bluebeam actually encodes one **+** a base rate on real JOB PLAN SETS — the measured 1.7% is agency STANDARDS documents, a different population, and its annotations are review markup rather than takeoffs |
...
| extended Microsoft-format metadata (OOXML extended-properties `AppVersion` [`XX.YYYY`] / `Company` / `Manager` / `Words` / `Pages` / `TotalTime` / `HiddenSlides` / `TitlesOfParts`; additional MAPI `.msg` props) | pure additive extraction the parsers already reach but **no consumer has asked for** — the Candidate-B lesson (recognition + the core fields already ship; more fields for their own sake is scope, not need). Surfaced by the MS Learn spec-audit (2026-07-14, `scratch/ms_learn_spec_audit_2026-07-14.md`); the spec gives each field's exact element + type up front | per-field prevalence in the OOXML/`.msg` corpus (how many `docProps` carry `Company`/`Manager`/etc.); the authoritative reference for each field's element/type is the **stable published spec** (ISO/IEC 29500 for OOXML; MS-OXPROPS / MS-OXMSG for MAPI) — cite the spec identifier + the value in-repo when it's load-bearing. Microsoft Learn is a convenient *lookup path* over those specs, not a controlled dependency or a release-gating oracle (unlike the vendored `exiftool` / pinned `pypdf`) — access to it isn't guaranteed forever, so capture the fact, don't depend on re-querying the service | a real consumer needs a specific field → promote **that field individually** (not the batch), at the v1.8.1 bar, provisional-first |
