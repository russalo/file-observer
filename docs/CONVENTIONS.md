...
| extended Microsoft-format metadata (OOXML extended-properties `AppVersion` [`XX.YYYY`] / `Company` / `Manager` / `Words` / `Pages` / `TotalTime` / `HiddenSlides` / `TitlesOfParts`; additional MAPI `.msg` props) | pure additive extraction the parsers already reach but **no consumer has asked for** — the Candidate-B lesson (recognition + the core fields already ship; more fields for their own sake is scope, not need). Surfaced by the MS Learn spec-audit (2026-07-14, `scratch/ms_learn_spec_audit_2026-07-14.md`); the spec gives each field's exact element + type up front | per-field prevalence in the OOXML/`.msg` corpus (how many `docProps` carry `Company`/`Manager`/etc.); the spec-oracle (Microsoft Learn: ISO/IEC 29500, MS-OXPROPS) is the authoritative reference for each field's element/type | a real consumer needs a specific field → promote **that field individually** (not the batch), at the v1.8.1 bar, provisional-first |
