...
|---|---|---|---|
| PDF markup annotations (`/Annots` count + subtype census) | needs a real markup-bearing specimen before a parser is written against a hypothesis rather than an encoding; and it is a BOUNDED-parse problem, not a field addition (one measured file carries **15,448** annotations on 89 pages — a naive walk is a cost and a DoS surface on untrusted input, so it earns the v1.8.1 bar from commit one) | **MEASURED 2026-08-07** (`scratch/pdf_markup_annotation_measurement_2026-08-07.md`): 654 real agency PDFs → **11 (1.7%)** carry markup annotations; producer-contains-Bluebeam **8/10 (80%)** vs **3/644 (0.47%)** otherwise = ~170× enrichment. Subtypes `/Square` 41,808 (Bluebeam AREA measurements render as `/Square`, so the census is near a measurement census) · `/Polygon` 20 · rest trivial. `/Link`+`/Widget`+`/Popup` excluded as navigation/form chrome | a markup-bearing Revu specimen supplied deliberately by CP (never rummaged from a consumer's corpus) **+** a base rate on real JOB PLAN SETS — the measured 1.7% is agency STANDARDS documents, a different population |
