...
Together, §12 → §11 → §13 → §14 are the "consume untrusted files safely" arc: **detect** (lexicon
screen) → **safe hand-off** (`--trusted-only`) → **audit bridge** (`--receipt`) → **route by tier**.

## 14. Routing on the signal: tiered consumption

→ [Example 11](../examples/11-tiered-routing/)

An all-or-nothing gate ("safe? yes/no") throws away most of what file-observer tells you. The per-file
signal — `safety_flags` plus *which* lexicon **category** hit — lets a consumer route each file to a
**tier** instead: **block** it (don't feed the model), send it for lighter **review**, or **pass** it
...
```bash
file-observer ./untrusted-uploads --specialists --lexicon terms.txt --receipt --stdout | your-router
```

Your router is a small, editable **policy map** — `{category → tier}` and `{safety_flag → tier}`,
most-severe wins, default `pass`. That map is *yours*: file-observer supplies the signal, you own the
decision (the same charter line as §13 — fo records what it *saw*, never what you *did*). Routing on the
receipt also closes the audit bridge — your block/review/pass decision points back at the exact
`receipt_id` file-observer emitted, so the two honest logs meet. Example 11 is the runnable version.

## Where to go next

- The [examples](../examples/) — runnable, one per concept (incl. [08 — the MCP server](../examples/08-mcp-server/), [09 — safe mode](../examples/09-trusted-only/), [10 — lexicon screen](../examples/10-lexicon-screen/), and [11 — tiered routing](../examples/11-tiered-routing/)).
