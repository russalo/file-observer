...
| PR bots (Codex / Gemini / Copilot) | ✅ **Gemini Code Assist HIGH (fixed):** a content block whose `type` is unhashable (list/dict in malformed JSON) raised `TypeError` on the `in CHATLOG_*_BLOCK_TYPES` membership check — a never-crash / bounded-observation violation (the v1.8.1 discipline). Fixed: a non-string `type` is coerced to `None` (matches neither set). Repro-confirmed before + after; `test_malformed_block_type_does_not_crash`. CI matrix (ubuntu/macOS/Windows + no-libmagic) all green. |

_(All four legs complete; findings logged in `scratch/review/v1.29_findings.md`.)_
